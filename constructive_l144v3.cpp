...
// ─── L144 V3 PROBE: boundary-aware compaction guard (ICCAD_BND_COMPACT_SAFE) ──
// Measured (constructive_l144.exe, ICCAD_BND_TRACE=1): on the SELECTED layout the
// boundary count goes 2069 -> 2149 across compaction, i.e. compaction is a NET
// PRODUCER of 80 boundary violations. It can do that because its acceptance test
// is csc_of() = (area + hw*hpwl) * exp(2*(bv+gf)/nsoft) -- bv sits inside an
// exponential that a big enough area win can outrun.
//
// Modes (0 = off = bit-identical):
//   1  FILTER : a candidate whose bv exceeds the pre-compaction layout's bv is
//               never considered. The iteration still runs, so a compaction that
//               is bv-neutral-or-better can still be taken.
//   2  VETO   : run compaction exactly as shipped, then discard the whole result
//               if its bv exceeds the pre-compaction bv (the literal "keep the
//               pre-compaction layout" reading).
//   3  FILTER+GF : mode 1 plus "does not increase group fragments either".
static int BND_COMPACT_SAFE = 0;
// Probe-only stderr counters (ICCAD_BND_CS_TRACE=1). Change no decision.
static bool BND_CS_TRACE = false;
...
static vector<XYWH> compact_layout(const vector<XYWH>& base){
    ...
    // L144 V3: pre-compaction reference counts. Computed only when the guard or
    // the probe trace is on => the shipped path is untouched.
    int cs_base_bv=0, cs_base_gf=0; long cs_rej=0;
    if (BND_COMPACT_SAFE || BND_CS_TRACE){
        cs_base_bv=count_boundary_violations(base);
        cs_base_gf=count_group_fragments(base);
    }
    ...
    auto consider=[&](vector<XYWH> c){
        if (BND_COMPACT_SAFE==1 || BND_COMPACT_SAFE==3){    // L144 V3 filter
            if (count_boundary_violations(c) > cs_base_bv){ cs_rej++; return; }
            if (BND_COMPACT_SAFE==3 && count_group_fragments(c) > cs_base_gf){
                cs_rej++; return; }
        }
        double sc=csc(c); if (sc<bsc){bsc=sc;best=c;} };
    ...
    // L144 V3 mode 2 (VETO): shipped selection, then throw the result away whole
    // if it costs boundary violations.
    bool vetoed=false;
    if (BND_COMPACT_SAFE==2 && count_boundary_violations(best)>cs_base_bv){
        best=base; vetoed=true;
    }
    if (BND_CS_TRACE){
        int ob=count_boundary_violations(best), og=count_group_fragments(best);
        fprintf(stderr,"CSAFE mode=%d bv %d %d gf %d %d rej=%ld veto=%d\n",
                BND_COMPACT_SAFE,cs_base_bv,ob,cs_base_gf,og,cs_rej,vetoed?1:0);
    }
    return best;
}
...
int main() {
    ...
    // L144 V3 probe knobs (0 / unset => bit-identical to constructive.exe)
    if (const char* e=getenv("ICCAD_BND_COMPACT_SAFE")){ int v=atoi(e); if (v>0) BND_COMPACT_SAFE=v; }
    if (getenv("ICCAD_BND_CS_TRACE")) BND_CS_TRACE=true;
