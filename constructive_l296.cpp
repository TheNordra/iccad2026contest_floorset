...
// ─── L296: the area/HPWL exchange rate in the selection proxies ──────────────
// The true per-case cost is
//     (1 + 0.5*max(0,hgap) + 0.5*max(0,agap)) * exp(2*V/nsoft)
//   = (0.5/A_label) * (area + (A_label/H_label)*hpwl) * exp(2*V/nsoft)
// so `area + hw*hpwl` has EXACTLY the right shape and hw should be the per-case
// ratio A_label/H_label.  Measured on the in-set 100 that ratio has median 290
// (range 25.6 .. 3095) while the shipped constant is 0.06 / 0.12 -- i.e. the
// hpwl term carries ~0.08 % of layout_score and the selector is, in practice,
// a pure area minimiser.
// A_label is estimated label-free by 1.035*sum(area_target) (the same estimator
// the Python pool proxy already uses and L114 validated); H_label by
// sqrt(A_hat)/(c*sum_w), a dimensional identity -- HPWL = total net weight x a
// characteristic length, and the characteristic length of a floorplan scales as
// sqrt(area).  Calibrated once on the in-set: c = 3.3675 predicts the true
// A_label/H_label to 5.2 % median / 12.6 % p90 error.
// Default LS_C = 0 keeps the shipped constant => bit-identical.
static double LS_C = 0.0;
static int    LS_MULT = 0;
// L296b: layout_score charges 150000 per boundary miss but only 6500 per cluster
// fragment, while the true cost counts them EQUALLY (both are +1 in V).  csc_of's
// own comment says so.  These two default to the shipped values.
static double LS_BV = 150000.0;
static double LS_GF = 6500.0;
static double _ls_hw_cache = -1.0;
static double compute_nsoft();
static double ls_hw(){
    double dflt = (N>=116)?0.12:0.06;
    if (LS_C <= 0.0) return dflt;
    if (_ls_hw_cache < 0.0){
        double sa=0.0; for (int i=0;i<N;i++) if (area_targets[i]>0) sa+=area_targets[i];
        double sw=0.0;
        for (const Edge&e:b2b_edges) if (e.w>0 && e.i>=0 && e.j>=0 && e.i<N && e.j<N) sw+=e.w;
        for (const Edge&e:p2b_edges) if (e.w>0 && e.j>=0 && e.j<N) sw+=e.w;
        _ls_hw_cache = (sw>1e-12 && sa>0.0) ? LS_C*sqrt(1.035*sa)/sw : dflt;
    }
    return _ls_hw_cache;
}
static double layout_score(const vector<XYWH>& p){
    ...
    double hw=ls_hw();
    if (LS_MULT) return (area+hw*hpwl)*exp(2.0*(bv+gf)/compute_nsoft());
    return area+hw*hpwl+LS_BV*bv+LS_GF*gf;
}
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_LS_C"))    { double v=atof(e); if (v>0) LS_C=v; }
    if (const char* e=getenv("ICCAD_LS_MULT")) { LS_MULT=atoi(e)?1:0; }
    if (const char* e=getenv("ICCAD_LS_BV"))   { double v=atof(e); if (v>0) LS_BV=v; }
    if (const char* e=getenv("ICCAD_LS_GF"))   { double v=atof(e); if (v>0) LS_GF=v; }
