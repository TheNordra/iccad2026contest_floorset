...
// ─── L144 PROBE-ONLY instrumentation (never shipped) ──────────────────────────
// ICCAD_BND_TRACE=1 prints boundary-violation counters to stderr. It changes NO
// layout decision: every counter is written from values the packer already
// computed, and nothing reads them back. Off => bit-identical to constructive.cpp.
static bool BND_TRACE = false;
// ─── L144 twin candidate: ICCAD_BND_EDGE_RUN ─────────────────────────────────
// The trace says the greedy takes a compliant (bp==0) slot 584001/584001 times
// it has one, and has none 21.7% of the time. So the miss is availability, not
// scoring: an earlier boundary item, pulled along its edge by the wire term,
// splits the edge into runs too short for the ones that follow. This makes
// boundary items fill their edge as a CONTIGUOUS run from the low corner.
//
// Deliberately built so it can only reorder COMPLIANT candidates: selection is
// lexicographic on (bp>0, score + run term), so a compliant slot can never lose
// to a non-compliant one no matter how large the weight. w=0 disables the whole
// path => bit-identical.
static double BND_RUN_W = 0.0;
static long TR_place = 0;       // boundary-item placements attempted
static long TR_avail = 0;       //   ... where a bp==0 candidate existed
static long TR_took = 0;        //   ... where the chosen candidate had bp==0
static long TR_prenudge = 0;    // violations right after pack_in_frame
static long TR_postnudge = 0;   //   ... after the three boundary repair passes
static long TR_frames = 0;      // run_frame calls counted into the two above
// ─── L144b PROBE-ONLY: WHO blocks the compliant slot ─────────────────────────
// Fires only inside `if (BND_TRACE)`, only on the 21.7% of boundary-item
// placements that ended with NO bp==0 candidate. Writes counters, reads nothing
// back, decides nothing. Off => bit-identical to constructive.cpp.
static long WH_fail   = 0;   // boundary items with no compliant candidate
// class (e): no exact-edge candidate was even generated / survived the bounds test
static long WH_e_nogen   = 0;
static long WH_e_conflict= 0;  //   ... because two members demand different coords
static long WH_e_toobig  = 0;  //   ... because the item does not fit in the frame
static long WH_e_clamp   = 0;  //   ... because the exact coord fell outside [0,max]
static long WH_e_other   = 0;
// classes (a)-(d): an exact-edge candidate existed but every one of them overlapped.
// Attributed on the LEAST-blocked compliant candidate, hardest blocker wins:
//   preplaced > same-side boundary > other-side boundary > plain block
static long WH_a_pre   = 0;
static long WH_b_same  = 0;
static long WH_c_diff  = 0;
static long WH_d_plain = 0;
static long WH_unexpl  = 0;
// per-blocking-BLOCK tally over the least-blocked compliant candidate
static long WB_pre=0, WB_same=0, WB_diff=0, WB_plain=0, WB_n=0;
// compound (cluster) items among the failures, and per class
static long WH_compound=0, WHc_e=0, WHc_a=0, WHc_b=0, WHc_c=0, WHc_d=0;
// corner (>1 required side bit) items among the failures
static long WH_corner=0;
// along-edge extent vs largest free run on that edge's strip, at this moment
static long WR_n=0, WR_fits=0, WR_frag=0;
static long WR_b00=0, WR_b25=0, WR_b50=0, WR_b75=0, WR_b100=0;
static double WR_sum=0.0;
// is the strip merely FRAGMENTED (total free >= extent) or genuinely FULL?
static long WR_capfit=0, WR_capshort=0;
// who occupies the strip: blocks intersecting it, by class
static long WS_n=0, WS_pre=0, WS_same=0, WS_diff=0, WS_plain=0;
...
struct M46Grid {
    ...
    // L144b PROBE-ONLY: same sweep, but reports WHICH rects overlap. Called only
    // from the BND_TRACE post-mortem, never from a placement decision. Bumping
    // qid is the same thing every overlaps() call does, so it cannot perturb a
    // later query.
    void collect(double x,double y,double w,double h,vector<int>& acc){
        ++qid;
        int x0=ixc(x), x1=ixc(x+w), y0=iyc(y), y1=iyc(y+h);
        for (int a=x0;a<=x1;a++) for (int b=y0;b<=y1;b++)
            for (int i:cells[a*G+b]){
                ...
                stamp[i]=qid;
                const XYWH& r=(*rp)[i];
                if (rect_overlap(x,y,w,h,r.x,r.y,r.w,r.h)) acc.push_back(i);
            }
    }
};

// ─── L144b PROBE-ONLY: post-mortem for a boundary item that found no bp==0 slot
// Runs after the candidate loop has already committed its choice, reads the same
// state the loop read, writes only WH_*/WB_*/WR_* counters. Never called unless
// ICCAD_BND_TRACE is set.
static const int WH_CAND_CAP = 512;      // exact-edge candidates probed per item
static long WH_capped = 0;
static void bnd_who_classify(const Item& it,
                             const vector<pair<double,double>>& cands,
                             const vector<XYWH>& out, const vector<char>& done,
                             M46Grid& g46, const vector<int>& owner,
                             double fw, double fh){
    WH_fail++;
    bool compound = it.blocks.size() > 1;
    if (compound) WH_compound++;
    // the sides this item must satisfy, and the exact origin coords they demand
    int code_all = 0;
    vector<double> reqx, reqy;
    for (size_t k=0;k<it.blocks.size();k++){
        int b=it.blocks[k], code=blocks[b].boundary; if (!code) continue;
        code_all |= code;
        double ox=it.offs[k].first, oy=it.offs[k].second;
        double bw=dims[b].first, bh=dims[b].second;
        if (code&B_LEFT)   reqx.push_back(-ox);
        if (code&B_RIGHT)  reqx.push_back(fw-ox-bw);
        if (code&B_BOTTOM) reqy.push_back(-oy);
        if (code&B_TOP)    reqy.push_back(fh-oy-bh);
    }
    int nbits=0; for (int m=1;m<=8;m<<=1) if (code_all&m) nbits++;
    if (nbits>1) WH_corner++;

    // --- along-edge extent of the item vs the largest contiguous free run on
    // the STRIP it would claim (L143's definition, measured at this instant).
    int bit = code_all & (-code_all);            // lowest required side bit
    bool lr = (bit==B_LEFT || bit==B_RIGHT);
    double ext   = lr ? it.h : it.w;
    double depth = lr ? it.w : it.h;
    double d0,d1;
    if      (bit==B_LEFT)   { d0=0.0;      d1=depth; }
    else if (bit==B_RIGHT)  { d0=fw-depth; d1=fw;    }
    else if (bit==B_BOTTOM) { d0=0.0;      d1=depth; }
    else                    { d0=fh-depth; d1=fh;    }
    double hi = lr ? fh : fw;
    vector<pair<double,double>> occ;
    for (int t=0;t<N;t++){
        if (!done[t]) continue;
        const XYWH& r=out[t];
        double ds = lr ? r.x : r.y, de = ds + (lr ? r.w : r.h);
        if (min(de,d1)-max(ds,d0) <= TOL) continue;      // outside the strip
        double s = lr ? r.y : r.x;
        occ.push_back({s, s + (lr ? r.h : r.w)});
        int tc=blocks[t].boundary;                       // who is in the strip
        WS_n++;
        if (blocks[t].is_preplaced) WS_pre++;
        else if (tc & code_all)     WS_same++;
        else if (tc)                WS_diff++;
        else                        WS_plain++;
    }
    sort(occ.begin(),occ.end());
    double cur=0.0, biggest=0.0, freetot=0.0;
    for (auto& iv:occ){ if (iv.first>cur){ biggest=max(biggest,iv.first-cur);
                                           freetot+=iv.first-cur; }
                        cur=max(cur,iv.second); }
    if (hi>cur){ biggest=max(biggest,hi-cur); freetot+=hi-cur; }
    if (freetot+TOL>=ext) WR_capfit++; else WR_capshort++;
    double ratio = biggest/max(ext,1e-12);
    WR_n++; WR_sum+=ratio;
    if (biggest+TOL>=ext) WR_fits++; else WR_frag++;
    if      (ratio<0.25) WR_b00++;
    else if (ratio<0.50) WR_b25++;
    else if (ratio<0.75) WR_b50++;
    else if (ratio<1.00) WR_b75++;
    else                 WR_b100++;

    // --- was an exact-edge (bp==0) candidate offered at all, and if so, who sat
    // on the least-blocked one?
    int best_n = 1<<30; bool have=false;
    vector<int> bset, bestset; int probed=0;
    for (auto& c:cands){
        double x=c.first, y=c.second;
        if (x<-TOL||y<-TOL||x+it.w>fw+TOL||y+it.h>fh+TOL) continue;
        if (item_boundary_penalty(it,x,y,fw,fh)!=0) continue;
        if (probed>=WH_CAND_CAP){ WH_capped++; break; }
        probed++;
        bset.clear();
        for (size_t k=0;k<it.blocks.size();k++){
            int b=it.blocks[k];
            double rx=x+it.offs[k].first, ry=y+it.offs[k].second;
            double bw=dims[b].first, bh=dims[b].second;
            vector<int> ri; g46.collect(rx,ry,bw,bh,ri);
            for (int i:ri){
                int t = (i<(int)owner.size()) ? owner[i] : -1; if (t<0) continue;
                bool dup=false; for (int q:bset) if (q==t){ dup=true; break; }
                if (!dup) bset.push_back(t);
            }
        }
        if (!have || (int)bset.size()<best_n){ have=true; best_n=(int)bset.size(); bestset=bset; }
    }
    if (!have){                                   // class (e)
        WH_e_nogen++; if (compound) WHc_e++;
        bool conflict=false;
        for (size_t i=1;i<reqx.size();i++) if (fabs(reqx[i]-reqx[0])>TOL) conflict=true;
        for (size_t i=1;i<reqy.size();i++) if (fabs(reqy[i]-reqy[0])>TOL) conflict=true;
        bool toobig = (it.w>fw+TOL)||(it.h>fh+TOL);
        double xmax=max(0.0,fw-it.w), ymax=max(0.0,fh-it.h);
        bool clamp=false;
        for (double v:reqx) if (v<-TOL||v>xmax+TOL) clamp=true;
        for (double v:reqy) if (v<-TOL||v>ymax+TOL) clamp=true;
        if      (conflict) WH_e_conflict++;
        else if (toobig)   WH_e_toobig++;
        else if (clamp)    WH_e_clamp++;
        else               WH_e_other++;
        return;
    }
    if (bestset.empty()){ WH_unexpl++; return; }  // would have been found: impossible
    bool ha=false, hb=false, hc=false;
    for (int t:bestset){
        int tc=blocks[t].boundary;
        if (blocks[t].is_preplaced){ ha=true; WB_pre++; }
        else if (tc & code_all)    { hb=true; WB_same++; }
        else if (tc)               { hc=true; WB_diff++; }
        else                       {          WB_plain++; }
        WB_n++;
    }
    if      (ha){ WH_a_pre++;   if (compound) WHc_a++; }
    else if (hb){ WH_b_same++;  if (compound) WHc_b++; }
    else if (hc){ WH_c_diff++;  if (compound) WHc_c++; }
    else        { WH_d_plain++; if (compound) WHc_d++; }
}
...
static bool pack_in_frame(double fw,double fh,const vector<Item>& items,vector<XYWH>& out){
    ...
    vector<int> tr_owner;                      // L144b probe: rect index -> block
    ...
    for (int i=0;i<N;i++) if (placed[i]){
        ...
        if (BND_TRACE) tr_owner.push_back(i);
    }
    ...
    for (const AnchoredCluster& ac:anchored_clusters){
        ...
        for (int b:mov){
            ...
            if (found){
                ...
                if (BND_TRACE) tr_owner.push_back(b);
    ...
    for (const Item& it:items){
        ...
        if (FREE_ASPECT>0 && it.blocks.size()==1){
            ...
            if (!blocks[sb].is_fixed && !blocks[sb].is_preplaced && blocks[sb].mib==0
                ...
                if (BND_TRACE) tr_owner.push_back(sb);
                continue;
            }
        }
        ...
        bool tr_any0=false, tr_took0=false, tr_isbnd=false;   // L144 probe only
        if (BND_TRACE)
            for (int b : it.blocks) if (blocks[b].boundary) { tr_isbnd=true; break; }
        int run_side=0; int best_cls=2;                       // L144 twin
        if (BND_RUN_W>0)
            for (int b : it.blocks) if (blocks[b].boundary) { run_side=blocks[b].boundary; break; }
        for (auto&c:cands){
            double x=c.first,y=c.second;
            if (x<-TOL||y<-TOL||x+it.w>fw+TOL||y+it.h>fh+TOL) continue;
            bool ov=false;
            ...
            double score=area+ANCHOR_W*ad+ww*WIRE_MULT*wire+BP_W*bp+1e-3*y+1e-4*x;
            if (BND_TRACE && tr_isbnd && bp==0) tr_any0=true;
            if (run_side){
                // contiguity term, and a lexicographic class so it can only
                // reorder candidates that are already compliant
                int cls = (bp>0) ? 1 : 0;
                double s2 = score;
                if (cls==0){
                    bool lr = (run_side & (B_LEFT|B_RIGHT)) != 0;
                    double t   = lr ? y  : x;
                    double ext = lr ? fh : fw;
                    s2 += BND_RUN_W * (t / max(ext,1e-9));
                }
                if (cls<best_cls || (cls==best_cls && s2<best)){
                    best_cls=cls; best=s2; bx=x; by=y; found=true;
                    if (BND_TRACE) tr_took0=(bp==0);
                }
                continue;
            }
        ...
        if (BND_TRACE && tr_isbnd){
            TR_place++; if (tr_any0) TR_avail++; if (tr_took0) TR_took++;
            // L144b: no compliant slot at all -> find out who is sitting on it.
            if (!tr_any0)
                bnd_who_classify(it,cands,out,done,g46,tr_owner,fw,fh);
        }
        if (!found) return false;
        for (size_t k=0;k<it.blocks.size();k++){
...
static void solve() {
    ...
    auto run_frame=[&](double fw,double fh,bool prev,const vector<XYWH>& pv,vector<XYWH>& out)->bool{
        ...
        use_prev=false;
        if (!ok) return false;
        if (BND_TRACE){ TR_prenudge += count_boundary_violations(out); TR_frames++; }
        final_boundary_nudge(out);
        final_group_boundary_nudge(out);
        final_single_edge_escape(out);
        if (BND_TRACE) TR_postnudge += count_boundary_violations(out);
        return true;
    };
    ...
    if (BND_TRACE) fprintf(stderr,"BNDTRACE sel_prepost %d",
                           count_boundary_violations(best));
    if (COMPACT) best=compact_layout(best);
    if (BND_TRACE) fprintf(stderr," %d",count_boundary_violations(best));
    ...
    if (BND_TRACE)
        fprintf(stderr,"BNDTRACE cand place=%ld avail=%ld took=%ld  "
                       "nudge pre=%ld post=%ld frames=%ld  final=%d\n",
                TR_place,TR_avail,TR_took,TR_prenudge,TR_postnudge,TR_frames,
                count_boundary_violations(best));
    if (BND_TRACE){
        fprintf(stderr,"BNDTRACE who fail=%ld compound=%ld corner=%ld capped=%ld "
                       "enogen=%ld econflict=%ld etoobig=%ld eclamp=%ld eother=%ld "
                       "apre=%ld bsame=%ld cdiff=%ld dplain=%ld unexpl=%ld\n",
                WH_fail,WH_compound,WH_corner,WH_capped,
                WH_e_nogen,WH_e_conflict,WH_e_toobig,WH_e_clamp,WH_e_other,
                WH_a_pre,WH_b_same,WH_c_diff,WH_d_plain,WH_unexpl);
        fprintf(stderr,"BNDTRACE blk n=%ld pre=%ld same=%ld diff=%ld plain=%ld  "
                       "cmp e=%ld a=%ld b=%ld c=%ld d=%ld\n",
                WB_n,WB_pre,WB_same,WB_diff,WB_plain,
                WHc_e,WHc_a,WHc_b,WHc_c,WHc_d);
        fprintf(stderr,"BNDTRACE run n=%ld fits=%ld frag=%ld "
                       "r00=%ld r25=%ld r50=%ld r75=%ld r100=%ld sum=%.6f "
                       "capfit=%ld capshort=%ld\n",
                WR_n,WR_fits,WR_frag,WR_b00,WR_b25,WR_b50,WR_b75,WR_b100,WR_sum,
                WR_capfit,WR_capshort);
        fprintf(stderr,"BNDTRACE strip n=%ld pre=%ld same=%ld diff=%ld plain=%ld\n",
                WS_n,WS_pre,WS_same,WS_diff,WS_plain);
    }
...
int main() {
    ...
    if (getenv("ICCAD_BND_TRACE")) BND_TRACE=true;      // L144 probe, stderr only
    if (const char* e=getenv("ICCAD_BND_EDGE_RUN")) { double v=atof(e); if (v>0) BND_RUN_W=v; }
