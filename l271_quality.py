...
ALL = {
    ...
    "ship":    {},
    # --- L271: is the hpwl damage from `nosize` repairable? -------------------
    # The greedy's wire term skips unplaced neighbours entirely (constructive.cpp
    # :1128 `else continue`), so reordering blinds it. Two order-INDEPENDENT
    # substitutes already exist in the binary; neither needs a new patch.
    #   HINT_MODE=2 routes anchor_dist() through the L137 GORDIAN hint for EVERY
    #   block, frame-relative, instead of only for blocks with no classic anchor.
    "ship_h2":     {"ICCAD_HINT_MODE": "2"},
    "nosize":      {"ICCAD_L268": "4"},
    "nosize_h2":   {"ICCAD_L268": "4", "ICCAD_HINT_MODE": "2"},
    "ship_gmed":   {"ICCAD_GUIDE_MED": "1"},
    "nosize_gmed": {"ICCAD_L268": "4", "ICCAD_GUIDE_MED": "1"},
    "nosize_wfa":  {"ICCAD_L268": "4", "ICCAD_WIRE_FOR_ALL": "1"},
    # --- the L271 retry (needs constructive_l271.exe) -------------------------
    # Retry the tightest frame that FAILED, in the density order, with the
    # successful layout scaled in as a wire guide. Two controls isolate which
    # half does the work: l271ship keeps the shipped ORDER (so only the guide is
    # new), l271ng drops the GUIDE (so only the order is new).
    "l271":      {"ICCAD_L271": "1"},
    "l271k":     {"ICCAD_L271": "1", "ICCAD_L271_K": "8"},
    "l271ship":  {"ICCAD_L271": "2"},
    "l271ng":    {"ICCAD_L271": "4"},
    "l271nr":    {"ICCAD_L271": "1", "ICCAD_L271_REF": "0"},
    # same-frame retry: antecedent is never empty (needs constructive_l273.exe)
    "l271s":     {"ICCAD_L271": "5"},
    "l271sng":   {"ICCAD_L271": "6"},
    "l271snr":   {"ICCAD_L271": "5", "ICCAD_L271_REF": "0"},
    "l271sngnr": {"ICCAD_L271": "6", "ICCAD_L271_REF": "0"},
    # L280: mutual-top-1 connectivity grouping (needs constructive_l280.exe)
    "l280":      {"ICCAD_L280": "1"},
    # --- L272: the L137 hint feeds the WIRE term (needs constructive_l272.exe) --
    # Zero extra packs. Order-independent, so it should help the density order
    # MORE than the shipped one -- that interaction is the whole hypothesis.
    "l272":        {"ICCAD_L272": "1"},
    "nosize_l272": {"ICCAD_L268": "4", "ICCAD_L272": "1"},
    "l271_l272":   {"ICCAD_L271": "1", "ICCAD_L272": "1"},
}
...
def main():
    ...
    for key in keys:
        ...
        def capture(extra, allow=None):
            ...
            final = None
            try:
                # 🚨 _solve_one's RETURN is the DEPLOYABLE layout: the wrapper's own
                # portfolio pick with _shape_lp_maybe already applied. Scoring only
                # the spy-captured raw stdout (what this harness did before, and
                # what l267_quality.py still does) reports a PRE-LP prize. The LP
                # works the AREA term hard and hpwl barely, so any mechanism that
                # buys area is partly a SUBSTITUTE for it and its deployable margin
                # is smaller than its gross one. This costs nothing -- the solve
                # already happened; the number was simply being thrown away.
                final = m67._solve_one(oc.MyOptimizer(verbose=False), lay)[0]
            finally:
                oc._run_profile = orig
            lpr = None
            if final is not None:
                try:
                    c = m67._cost(final, lay)
                    lpr = dict(cost=float(c.cost), feas=bool(c.is_feasible),
        ...
        capped = {k: capture(v, GATE.get(k)) for k, v in ARMS.items()}
        caps = {k: v[0] for k, v in capped.items()}
        lps = {k: v[1] for k, v in capped.items()}
    ...
    have_lp = all((nm + "_lp") in rows[0] for nm in ARMS)
    if have_lp:
        print()
        print("  DEPLOYABLE (post shape-LP, the wrapper's own returned layout):")
        if "ship" in ARMS:
            lb = wm(lambda r: r["ship_lp"])
            print("    {:12s} {:.6f}".format("ship", lb))
            for nm in ARMS:
                if nm == "ship":
                    continue
                w = wm(lambda r: r[nm + "_lp"])
                print("    {:12s} {:.6f}   {:+.4f}%   hpwl {:.4f} area {:.4f} vrel {:.4f}"
                      "   feasible {}/{}".format(
                          nm, w, 100 * (w - lb) / lb,
                          wm(lambda r: r[nm + "_lphg"]), wm(lambda r: r[nm + "_lpag"]),
                          wm(lambda r: r[nm + "_lpvr"]),
                          sum(1 for r in rows if r[nm + "_lpfeas"]), len(rows)))
            print("    ship gaps: hpwl {:.4f} area {:.4f} vrel {:.4f}".format(
                wm(lambda r: r["ship_lphg"]), wm(lambda r: r["ship_lpag"]),
                wm(lambda r: r["ship_lpvr"])))
    print()
    ...
    print()
    print("  DETERMINISTIC pack bill (max-setter profile / whole pool):")
    if "ship" in ARMS:
