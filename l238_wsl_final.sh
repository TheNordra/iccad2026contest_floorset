#!/usr/bin/env bash
# L238 -- Linux verify of the L234+L235 package:
...
export L117_WORK=$HOME/l238
TAR=$R/build_submission/cadc1075.tar.gz
cd "$R" || exit 1
mkdir -p "$L117_WORK"

echo "== L238 Linux verify -- L234+L235 package  $(date -u +%FT%TZ)  nproc=$(nproc)"
...
[ $? -eq 0 ] || FAIL=1
if [ "$FAIL" -ne 0 ]; then
  echo "L238 ABORT: the tar is not the package we think it is."; exit 1
fi
...
echo; echo "########## BUDGET ##########"
"$V" l153_budget.py "$BASE" "$CTRL" | tee "$R/l238_budget_linux.txt"
B=$(grep '^BUDGET' "$R/l238_budget_linux.txt" | awk '{print $2}')
echo "   budget = $B"
...
echo; echo "L238_DONE $(date -u +%FT%TZ)"
