...
def _shape_lp(pos, block_count, area_targets, b2b_connectivity,
    ...
    try:
        _hbs = float(os.environ.get("ICCAD_LP_HB_SCALE", "1") or "1")
    except ValueError:
        _hbs = 1.0
    _hb = float(prev["hpwl"]) * _hbs
    # PROBE (L166 lens A): predict the evaluator's hpwl_baseline from the INPUTS
    #   hb ~= K * sqrt(sum area_targets) * (sum of all net weights)
    # K = 0.2994 fitted on the 100 in-set labels; on 240 held-out floorset_lite
    # layouts the same K lands at ratio p10 0.916 / p50 0.995 / p90 1.090.
    _K = os.environ.get("ICCAD_LP_HB_PRED", "")
    if _K not in ("", "0"):
        try:
