...
# LENS D probe (offline, untracked): 32 UNIFORMLY RANDOM knob-cloud vectors,
# gated OFF by default so the file is bit-identical to shipped unless
# ICCAD_LENSD_POOL=1. Random, not greedy: an unbiased end-to-end read of what
# pool additions are worth AFTER the L147/LP refinement stage.
_LENSD_EXTRA = [
 {
...
def _pool_indices(block_count: int) -> List[int]:
    ...
    extra = ((_M55_IDX if m55 else frozenset()) | esc
             ...
             | _l137_active(block_count)
             | _lensd_active(block_count))         # LENS D probe tier
