...
static bool pack_in_frame(double fw,double fh,const vector<Item>& items,vector<XYWH>& out){
    ...
    for (int i=0;i<N;i++) if (placed[i]){
        ...
        rects.push_back(pos[i]); rect_own.push_back(-1); g46.add((int)rects.size()-1); bbox_add(pos[i].x,pos[i].y,pos[i].w,pos[i].h); done[i]=1;
    }
    ...
    for (const AnchoredCluster& ac:anchored_clusters){
        ...
        for (int b:mov){
            ...
            if (found){
                out[b]={bx,by,bw,bh}; rects.push_back({bx,by,bw,bh}); rect_own.push_back(-1); g46.add((int)rects.size()-1);
                bbox_add(bx,by,bw,bh); done[b]=1; cluster_rects.push_back({bx,by,bw,bh});
            }
        }
    }
    vector<int> l262_work; l262_work.reserve(items.size()*2);
