...
// ─── M53 L2 PROBE ONLY (this file is an offline copy; never shipped) ─────────
// Gated stochastic noise for best-of-N sampling. sigma=0 (default) performs
// ZERO rng draws and leaves every expression bit-identical to constructive.cpp.
static double NOISE_SIGMA = 0.0;   // ICCAD_NOISE: per-candidate multiplicative score noise
static double NOISE_WSIGMA = 0.0;  // ICCAD_NOISE_W: per-run log-uniform jitter on ANCHOR_W/WIRE_MULT/BP_W
static unsigned long long g_rng_state = 0x9e3779b97f4a7c15ULL; // set from ICCAD_SEED in main()
static inline double rng_u(){       // xorshift64* -> uniform in (0,1)
        ...
        for (int b:mov){
            ...
            for (double r:ratios){
                ...
                for (auto&c:cands){
                    ...
                    if (NOISE_SIGMA>0) score *= (1.0 + NOISE_SIGMA*(2.0*rng_u()-1.0)); // M53 L2 probe
...
int main() {
    ...
    // M53 L2 probe knobs: parse + seed ONCE per process (pack_in_frame is
    // re-entered many times per run; a single stream keeps same-seed runs
    // exactly reproducible). splitmix64 decorrelates adjacent seeds.
    if (const char* e=getenv("ICCAD_NOISE"))   { double v=atof(e); if (v>0) NOISE_SIGMA=v; }
    if (const char* e=getenv("ICCAD_NOISE_W")) { double v=atof(e); if (v>0) NOISE_WSIGMA=v; }
    if (const char* e=getenv("ICCAD_SEED")){
        unsigned long long z=strtoull(e,nullptr,10);
        z += 0x9e3779b97f4a7c15ULL;
        z = (z ^ (z>>30)) * 0xbf58476d1ce4e5b9ULL;
        z = (z ^ (z>>27)) * 0x94d049bb133111ebULL;
        z ^= z>>31;
        if (!z) z=0x9e3779b97f4a7c15ULL;
        g_rng_state=z;
    }
    if (NOISE_WSIGMA>0){   // mechanism A: continuous profile-space sampling
        ANCHOR_W  *= exp(NOISE_WSIGMA*(2.0*rng_u()-1.0));
        WIRE_MULT *= exp(NOISE_WSIGMA*(2.0*rng_u()-1.0));
        BP_W      *= exp(NOISE_WSIGMA*(2.0*rng_u()-1.0));
    }
