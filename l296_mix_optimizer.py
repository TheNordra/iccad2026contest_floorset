...
# L296 MIX ARM -- a COPY of optimizer_constructive.py with two table
# defaults changed and nothing else.  `_L196_LPGATE` is all 1s (the LP
# runs on every block count) and `_L157_DEPTH` is 2 on the old 1-set /
# 1 on the old 0-set (a second pass only where L196 judged the case can
# absorb it).  `_depth_ok`'s shipped path is `pass_no <= _L157_DEPTH[n]`,
# deterministic and per block count, so every bit-equality gate still
# works.  Generated by hand from the tree wrapper; NOT a modification
# of it.
