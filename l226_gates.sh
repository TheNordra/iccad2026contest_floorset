#!/bin/sh
# L226 -- in-set gates for the L196 tree WITH ROUTE A OFF + the L211 pool drop + the L223 REFINE band
#
...
set -u
LOCK=/c/ICCAD_ml/ship_final/.l226.lock
...
run () {
  tag="$1"; shift
  rm -f "../l226_${tag}_stats.txt"
  env ICCAD_ADAPTIVE_CORES=48 ICCAD_SHAPE_LP_STATS="../l226_${tag}_stats.txt" "$@" \
    "$PY" -u iccad2026_evaluate.py --evaluate ../optimizer_constructive.py \
    -o "../results_L226_${tag}.json" > "../l226_${tag}.log" 2>&1
  rc=$?
  n=$(wc -l < "../l226_${tag}_stats.txt" 2>/dev/null || echo 0)
  echo "$tag exit=$rc  LPran=${n}  SAfallback=$(grep -c 'SA fallback' "../l226_${tag}.log")  scipy=$(grep -c 'scipy] source=' "../l226_${tag}.log")"
}
run det1
run det2
