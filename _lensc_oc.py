...
def _lensc_dump():
    import os as _o
    _p = _o.environ.get("ICCAD_LENSC_LOG", "")
    if _p:
        _json.dump(_LENSC_LOG, open(_p, "w"))
    ...
    def _chain(kw):
        ...
        for _it in range(max(1, iters)):
            ...
            if better and not worse:
                _hok = int(bool(hard_ok(P0, newP, key)))
            _LENSC_LOG.append((int(block_count), _it + 1,
                               prev_["hpwl"], prev_["area"], prev_["vrel"],
                               m["hpwl"], m["area"], m["vrel"],
                               int(bool(better)), int(bool(worse)), int(_hok),
                               float(_sumA_lensc)))
            if not (better and not worse and _hok == 1):
                break
            P_, prev_ = newP, m
        return P_
