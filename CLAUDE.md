...
### 🏆 最佳：local 1.3277 / 投影 real @12c M=11 1.4742→0.9757（M41+M42+M46 累計 ~−34%）再疊 M45 universal ≤−1.6% + 低核 @4c −9% + **M47 wrapper overhead 歸零（avg 2.70→1.85s、實測 t 貼齊模型投影）**（2026-07-04；100/100 feasible）
...
> quality 軸 converged；**RF 軸已 ship 五槍**（M41 swap / M42 big-redundant / M45 band tiers + cores-adaptive / M46 hot-path exact / M47 wrapper overhead）。依 ROI：
...
4. **M48 hardening 候選（未做，plan `session-eager-summit`）**：(a) `_ensure_compiled` 加 `clang++`/`-O2` fallback 鏈（官方機編譯失敗 → 靜默退 SA ≈ 總分 ×2+ 災難，Alpha 過但 tail-risk 值得堵）；(b) `solve()` blanket try/except → SA fallback（hidden 怪案例外 → evaluator 記 M_PENALTY 10.0）；(c) Beta 冷啟動 dry-run（刪 .exe → import 編譯 → 跑 3 案；編譯在 `_load_optimizer` 內、**計時窗外**，`evaluate.py:845` vs `:884`）。全部零 local 行為變更。
...
- `optimizer_constructive.py` 🏆 — 40-prof portfolio + shapely proxy(_RH=1.4)；pruned profile 標 `[Mxx-pruned]` 註解可復原；**M41+M42+M45 `_pool_indices()` default-on 四階砍 profiles：swap + `_BIG_REDUNDANT_IDX`（n>100）+ `_M45_BAND_DROP`（universal 中案 band）+ `_M45_LOWCORE_DROP`（`_effective_cores()≤8` 才啟用）**；**M47 proxy 路徑：`as_completed` 就地算 proxy（一次一個，⚠️ 並發=GIL thrash 4× 慢）+ `_hpwl_b2b_fast`/`_hpwl_p2b_fast` tolist 版（逐位=harness 版，動它必重過 `m47b_proxy_equiv.py`）**；旋鈕見 env 節
