...
1. **RF 軸殘留 = wall-setter 本身（M46 候選）**：M45 後高核 regime 的 wall 由各案 max-setter build stack（#18/#2/#17 等 6.9-8.5s FREE/CA/FC）設底——pool 砍法已到底（(100,∞] 合併 band candidates=0）。再降只能 **C++ 加速 max-setter 本身**（frame 早剪枝 / free-search bound / PGO；`constructive.cpp` 無 stage timing，需先加 gated 計時 probe）。量級：α 倍加速 → real ×α^0.3（20% → −6%/計分案）。⚠️ 2026-07-02 已驗證死路：**cores=12 假設下的三階砍法增益恰 0.00%**（`[[teammate-prune-v1-zero-gain]]`）——但那是「高核+砍非-wall-setter」的特例；M45 tier-4 已 cover 低核 regime（−9.3% @4c）。任何新 pool-prune 仍須過 `rf_score_model.py` M45 strict gate。
