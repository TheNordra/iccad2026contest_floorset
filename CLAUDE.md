...
4. **RuntimeFactor 軸（M41+M42+M45，三槍已 ship）**：以上三點全是 **quality**；計分式還有 `max(0.7,R^0.3)` 一項，本地 eval 強制 =1.0 而被全程忽略。`cost∝t^0.3` + 大案 quality 卡死 → 砍 wall = median-independent real-score 增益。**M41** 砍 swap（投影 ~−12%）；**M42** 砍 21 隻 n>100 build 冗餘（per-big-n；wall 砍半、疊加 −22% @M=11）；**M45**（2026-07-02）把 redundancy 從累積（n>T）推廣到 **band-scoped**（不贏任何 lo<n≤hi 案）：中案 band 冗餘的大案 winner（#2/#13/#17/#18）在中案是純 sum 死重 → tier-3 universal 砍 9 隻（中案 12c 也 sum-bound → −1.2~1.5% ∀cores）+ tier-4 低核（cores≤8）深砍（@4c −9.3%、slowness uncapped 的下行保險）。M45 gate 升級為 **strict selection-preserving**（逐案 cost 相等，比 WIN 不等式強——後者在 RF floor 附近高估）→ 任何 median、任何 cores 弱贏。官方 cores/median 皆未知（PDF 無 specs）→ cores 偵測 fail-open 回 shipped 行為。見 `[[m45-band-tiers]]`、`[[m42-runtime-2nd-order]]`、`[[m41-runtime-factor]]`。

## 未來發展方向（M45 後）

> quality 軸 converged；**RF 軸已 ship 三槍**（M41 swap / M42 big-redundant / M45 band tiers + cores-adaptive）。依 ROI：

1. **M46 = C++ 加速 wall-setter 本身（2026-07-03 分析完成、lever 已原型驗證 α≈0.475、⏳ 待 ship，見 `[[m46-wall-setter-speedup]]`）**：M45 後 20 個 n>100 案 @12c 全 MAX-bound、max-setter 全是 FREE-stack（#18×6/#2×5/#23×4…、max1≈max2 → 須加速共同熱路徑）。gated 計時 probe（`constructive_m46.exe`、`ICCAD_STAGE_TIMING=1`）實測：**pack≈98% of wall、其中 `item_candidates` 候選生成 66%（set/dedup/sort）+ overlap 掃描 29% + wire 7%**。三個 exact（建構上 bit-identical）優化已在副本原型 + 12 組 verify byte-identical：**opt-A** 跳過 interior item 永遠不讀的 xs/ys set 插入、**opt-B** dedup 換 hash（保 first-occurrence 順序）、**opt-C** 32×32 uniform grid overlap（布林等價：真重疊 >TOL → 必共 cell）。**A+B: α≈0.62；A+B+C: α≈0.46–0.50**、**verify100 600/600 byte-identical**（100 案 × 6 path-代表 profiles 含 OS/FA/FC/CA；per-band α：小案 0.71 → 大案 (100,120] **0.49**，計分案拿最大加速）→ 投影 real ≈ **−16% @M=11**（`rf_score_model.py` M46 區塊 α=0.5 行：@12c −16.1%、∀cores/M∈[6,14] 穩健、僅 M=20 高核衰減至 −3.9%；純加速 = t'≤t ∀案 → **∀median ∀cores 無條件弱贏**）。差分交叉驗證吻合（noREFINE −87.7% ≈ 儀裝 87.9%、wall ∝ pack 數）。**opt-D（refine 固定點早退）RED 勿重試**：12 pass 內從不逐位收斂（`refine_fixed=0` 全組）→ refine 砍次數只剩 quality-affecting tier。⚠️ 地雷：候選 sort 用 TOL 比較器 + std::sort 不穩定 → **候選生成順序與 sort 絕不可動**；只有布林/結構工作可加速。PGO 不採用（wrapper 現場編譯無法兩段 build + `[[codex-m43-lto-inert]]`）。**Ship 管線（下一步）**：A/B/C 併入 `constructive.cpp`（不帶計時儀裝）→ 重編 → 重跑 `profile_audit.py`（~1.5h）→ 與舊 cache byte-compare positions → 全同則 drop 常數不變、僅刷新投影；有差則完整 regen + 官方 eval 確認 1.3277。⚠️ 2026-07-02 已驗證死路：**cores=12 假設下的三階砍法增益恰 0.00%**（`[[teammate-prune-v1-zero-gain]]`）——但那是「高核+砍非-wall-setter」的特例；M45 tier-4 已 cover 低核 regime（−9.3% @4c）。任何新 pool-prune 仍須過 `rf_score_model.py` M45 strict gate。
