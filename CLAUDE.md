...
- **現況：我們 1.3313（M35）已反超組員所有 legit 版本（~17.8%）。packer/order lever 自 M26-27 枯竭，但 free-aspect 軸 M29 重開 → ratio/boundary/cluster-uniform/cluster-per-member/anchored-per-member 五子軸全 ship（M29-M35）。剩餘 headroom = reconstruction**（RED，M29 從零 builder 8.22 證偽），需要的是重寫 placer 架構而非更強 optimizer。
...
### 🏆 最佳：Total Score = 1.3313（M35 anchored-cluster free-aspect, 38-prof, 2026-06-21；avg 9.30s, est_wall ~7.5s OS16-bound 不變, 100/100 feasible）
...
### 📋 NEXT SESSION 計畫：free-aspect 五子軸全 ship → 殘渣 follow-up / RED reconstruction / 收束

**幾乎所有 in-env optimization lever 枯竭**：packer/order（M26-27 天花板）+ free-aspect 五子軸（ratio M31 / boundary M32 / cluster-uniform M33 / cluster-per-member M34 / anchored-per-member M35）全 ship 或死路。proxy/_RH 完美（零漏分）。
⚠️ **M35 教訓：「預測會死」≠ 死**——anchored free-aspect 本被類比 M32 FREE_BOUNDARY 判 <0.05%（同 packing-score 仲裁、無 cluster layout-key），實際 +0.111%，因 anchored first-pass 的 score 多了 `bp`+keep-connected 項、仲裁結果與純 FREE_BOUNDARY 不同。**bounded probe 值得做，別只憑類比預判死路。**
**剩餘方向依 ROI**：
1. **anchored free-aspect 殘渣（cheap，先試）**：FREE_ANCHORED 目前重用 `FREE_RATIOS`（max 2.0），但 M34 cluster-member 共振峰在 **4.0** → **試寬 ratio**（per-frame ∝ frames → 須查 wall）；+GM/+OS 其他 stack；mixed cluster 的 boundary!=0 成員目前 gate 掉。預期小但 M35 已證殘渣會翻盤。
...
3. **收束本題**：1.3313 領先組員所有 legit 版本 ~17.8%，runtime 安全（avg 9.30s）。若無新 insight，合理停點。
...
89 **~1.554**（M35 anchored 從 M34 的 1.5640 再降，仍最高；preplaced boundary 撐壞 outline）、85 ~1.536（M32 LR4.5）、65 **~1.690**（M35 從 1.7325）、62 1.5227、88 ~1.412（M34）、82 ~1.363（M34）、52 **~1.361**（M35 從 1.4071）、79 ~1.247（M34）、66 ~1.270（M34）、91 1.3481。硬 case（89/85/62）= preplaced boundary 幾何強迫；**M33→M34→M35 證實成員形狀（cluster-member + anchored-member aspect）+ tight frame 能顯著鬆動 89/82/88/79/66/65/52**（非 packer 架構問題，是成員形狀；anchored 成員貼牆形狀也能優化，推翻「自由度低」預判）。
...
- `optimizer_constructive.py` 🏆 — 38-prof portfolio + shapely proxy(_RH=1.4)；M25 剪 18 條（`[M25-pruned]` 註解可復原）
