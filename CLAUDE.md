...
- **現況：M41（2026-06-25）開全新 RuntimeFactor 軸（與 quality 正交，被本地 eval 強制 RF=1.0 藏住）；M42（2026-06-26）第二槍；M45（2026-07-02）第三槍 per-band tiers + cores-adaptive；M46（2026-07-03）第四槍 C++ hot-path exact 加速**。本地分 1.3269→**1.3277**（RF=1.0 fiction，+0.06%、之後恆定）：M41 砍 6 隻 swap profiles（OS16×3/OS8×2/OM8，大案 18-20s 純 runtime 死重）→ avg 9.89→5.90s；**M42 再砍 21 隻「不贏任何 n>100 案」的 build profiles（per-big-n 冗餘，非全域 LOO）→ avg 5.90→4.73s（−20%）、big-case wall n=120 砍半 15.6→8.0s、quality BIT-IDENTICAL（local 仍 1.3277、20 案全 median-independent WIN）**、投影 real 疊加 M41 共 **~−22%**（@ M=11、robust 跨 M∈[6,20]）、100/100 feasible；**M46 再 exact 加速 pack 熱路徑（opt-A/B/C，全池 4100/4100 positions bit-identical）→ avg 4.73→2.70s、@12c M=11 投影 1.1473→0.9757（再 −15%）、零下行（純加速無 +0.06% local 代價）**。**quality 軸仍 converged**（packer/order M26-27 + free-aspect 六子軸 M29-M37 + reconstruction M40-RED 全探畢）；RF 軸唯一前提是「官方真套 RF」（否則只損 +0.06%，極不對稱賭注）。逐版細節 git log + memory（`[[m46-wall-setter-speedup]]`、`[[m45-band-tiers]]`、`[[m42-runtime-2nd-order]]`…`[[m29-tree-decoder]]`）。
...
### 🏆 最佳：local 1.3277 / 投影 real @12c M=11 1.4742→0.9757（M41+M42+M46 累計 ~−34%）再疊 M45 universal ≤−1.6% + 低核 @4c −9%（2026-07-03；100/100 feasible、avg **2.70s**）
...
- ⚠️ **eval 實際 ~6 分鐘（M46 後）**（100 案 serial、avg 2.70s，大案 n>110 ~4-6s；M42 時代 ~8 分、M41 前 ~15-17 分）；background 用 harness `run_in_background`（完成通知）；`> file 2>&1` 在 PowerShell 對 native exe 印 cosmetic `NativeCommandError`（無害）
...
- `optimizer_constructive.py` 🏆 — 40-prof portfolio + shapely proxy(_RH=1.4)；pruned profile 標 `[Mxx-pruned]` 註解可復原；**M41+M42+M45 `_pool_indices()` default-on 四階砍 profiles：swap + `_BIG_REDUNDANT_IDX`（n>100）+ `_M45_BAND_DROP`（universal 中案 band）+ `_M45_LOWCORE_DROP`（`_effective_cores()≤8` 才啟用）**；旋鈕見 env 節
