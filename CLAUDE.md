...
1. **🚨 換件（deadline 前）**：把 M71 打包進 tar。⚠️ 必查 **tier-5 在 M71 下是否還是 in-sample no-op**——它在 ≥40 核放回 M42 那 22 隻，而 M71 正好讓那 22 隻不再冗餘（與第 2 點同根因），舊的「強制 48c 逐位相同」可能已失效（多半變好，但 48c wall 代價要用 `m67e_rf48.py project --idx-bands big` 重估）。
2. **adaptive stack 常數 regen（~0.84%，桌上最大已知肉）**：`profile_audit.py` 產新 `audit_cache.pkl` → `rf_score_model.py` 重算 `_BIG_REDUNDANT_IDX`/`_M45_BAND_DROP`/`_M45_LOWCORE_DROP` → `m49_refine_probe.py` 三 gate 重 pin。⚠️ **必須在 REFINE K=4/K=8 overlay 下做**（M67-F 更正 B：strict gate 在出貨組態已失效），不可直接吃 `audit_cache.pkl` 的 K=12 positions。
3. **M71 剩下四個 gated-off 旗標**（`CLUSTER_BND_CORNER` / `ANCHORED_BND_REPACK` / `CLUSTER_BND_PERMUTE` / `HPWL_SAFE_CLUSTER_SLIDE`）：組員 M55 用 PERMUTE+EDGE_PACK+SLIDE 在舊引擎拿 1.2984（但 10s/case ⇒ RF 死）。各自單開量一次、bar 0.05% 加權。這是 §戰略結論 5 指的同一條軸，目前最便宜的 quality 探索。
...
- **offline 旋鈕（永不送件）**：`ICCAD_L1_POOL=1`（84 池，須配 `ADAPTIVE_POOL=0` + `PROFILE_TIMEOUT=600`）、`ICCAD_M67F_RESTORE=1`（只跳過 M42+tier-3，量 θ 用）
...
- **`m67_oos_probe.py`** — OOS 泛化（`gate0/run/report/ref/pool0/restore`，`--pool0-lo/-hi` 選帶）；`m67_oos_cache.pkl`
