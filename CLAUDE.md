...
- **現況：我們 1.3269（M37）已反超組員所有 legit 版本（~18.1%）。packer/order lever 自 M26-27 枯竭，但 free-aspect 軸 M29 重開 → ratio/boundary/cluster-uniform/cluster-per-member/anchored 五子軸全 ship（M29-M35），M36 榨乾 anchored 殘渣（寬 ratio + ungate boundary），M37 再開 MIB-member 第六子軸（wide MIB reshape 疊 anchored，破最難案 89→1.5232，又一個「預判死」翻盤）。M38 收束探：MIB 殘渣（gated-recipe + 寬 ratio）探畢全 below bar。M39 最後對稱殘渣探：`FREE_CLUSTER_BND`（pure-movable cluster 的 boundary 成員 ungate，鏡像 M36、layout-key 仲裁、wall-free）liveness 極高（92/100、391 members）且翻盤方向對（非 M32 純死），但贏案全低權重、硬案零移動 → best +0.034% below bar、revert。free-aspect 六子軸家族 CLOSED、鎖定 1.3269。OS16×free 確定 runtime-死。剩餘 headroom = reconstruction**（RED，M29 從零 builder 8.22 證偽），需要的是重寫 placer 架構而非更強 optimizer。
...
### 🏆 最佳：Total Score = 1.3269（M37 MIB-aspect wide-reshape, 40-prof, 2026-06-22；avg 9.89s, est_wall OS16-bound 不變, 100/100 feasible）
...
### 📋 狀態：free-aspect 家族 CLOSED（M29-M39）→ RED reconstruction or 收束

**所有 wall-safe in-env optimization lever 枯竭**：packer/order（M26-27 天花板）+ free-aspect 六子軸（ratio M31 / boundary M32 / cluster-uniform M33 / cluster-per-member M34 / anchored M35-M36 / **MIB-member M37**）全 ship 或死路。proxy/_RH 完美（零漏分）。**M38（2026-06-23）：MIB 殘渣最後一探（gated complement recipe sweep）→ 全 below bar（峰值 79 +0.033% < 0.05%）→ free-aspect 家族確定 CLOSED、收束於 1.3269。M39（2026-06-23）：最後一個對稱殘渣 `FREE_CLUSTER_BND`（pure-movable cluster 的 boundary 成員 ungate，鏡像 M36 anchored-BND、用 M34 layout-key 仲裁、build-time wall-free）探畢——liveness 極高（92/100 案、191 groups、391 members，含全部硬案 89/97/85/82/88），且翻盤方向正確（layout-key 確實挑更好 aspect，異於 M32 greedy-area 的 0.000% 純死），但贏案全低權重小/中案、硬案零移動 → 三 stack 全 below bar（best fc_gm+BND +0.034%、fc_pin+BND +0.023%、anc_mib+BND +0.017%）→ revert、收束。連續第二個 genuine below-bar（M38/M39），「預判死翻盤」三連（M35/M36/M37）結束。** 下一個有意義動作只剩 RED reconstruction（須重寫 placer，非加 lever）。
⚠️ **M35→M36→M37 教訓（三度應驗）：「預測會死」≠ 死**——anchored 三殘渣（M35/M36）+ MIB uniform 小群（M37）都被類比 M32 FREE_BOUNDARY 判死，全翻盤（疊 anchored score 的 `bp`+keep-connected 放大）。**bounded probe 值得做（先 liveness 數可動結構、再 stack 探放大），別只憑類比預判死路。** 但 **runtime-coupled 的就是死**（M36 OS16×free +0.630% 但 48s，與 4× runtime 不可拆，見死路 ledger）。
**剩餘方向依 ROI**：
1. ~~**MIB-member aspect**~~ **M37 SHIPPED 1.3269** + **M38 殘渣探畢 CLOSED**（wide MIB reshape 疊 anchored，破最難案 89→1.5232）。M38 釐清：「per-group MIB 搜尋」殘渣**不需新 arbiter**——portfolio wrapper 本身即 per-case 選擇器 → 問題化簡為「有無單一 MIB profile 過 0.05% bar」。`anchored_gated` recipe（去 FREE_ANCHORED_BND）sweep 全 below bar（峰值 79 gated-6.0 +0.033%、7.0+ 回落；已 ship 的 89/61 在 gated +0.000%、無 disjoint 新案）→ **free-aspect 六子軸全榨乾**。
2. **RED reconstruction**（重寫 placer 為 netlist-driven slicing-tree + free aspect）：headroom 仍大（oracle 1.1079，剩 +0.219），但 **M29 從零 connectivity builder 已證偽 8.22 vs 1.38（0/100）** → 須重寫 constructive.cpp 全部約束機制（boundary/grouping/MIB/精度），非加 lever。ROI 極低、風險極高。
3. **收束本題**：1.3269 領先組員所有 legit 版本 ~18.1%，runtime 安全（avg 9.89s）。free-aspect 六子軸已盡，若無新 insight，合理停點。
...
- **MIB-member aspect 飽和邊界（M37 ship + M38 收束，勿重掃）**：uniform `ICCAD_MIB_ASPECT`（共享方形→共享同面積矩形）共振峰 = ungated-wide **5.0**（89/61，已 ship M37）；tall 0.25→79 +0.027% below bar。**M38 gated complement sweep**（`anchored_gated`= fc_anchored_wide_pin_tight 去 FREE_ANCHORED_BND，[dbg_mib_sweep.py](dbg_mib_sweep.py) 加此 recipe）：唯一 mover 仍是 case 79（gated-**wide** 共振 **6.0** +0.033%、7.0+ 回落不過 bar），已 ship 的 89/61 在 gated +0.000%、無 disjoint 高價值案。**per-case 自動選 ratio 不需 arbiter**（portfolio wrapper 已是 per-case 選擇器）→ 此路即「有無單一 MIB profile 過 0.05% bar」，全 below bar。**free-aspect 六子軸家族 CLOSED**，剩 RED reconstruction 或收束
- **FREE_CLUSTER boundary-ungate（`ICCAD_FREE_CLUSTER_BND`，M39，勿重掃）**：對稱於 M36 anchored-BND——讓 pure-movable cluster 的 boundary 成員也在 make_group_item 搜 aspect（M34 **layout-key** 仲裁、build-time **wall-free**）。**liveness 極高**（92/100 案、191 groups、391 members，含全部硬案 89/97/85/82/88，[dbg_fcbnd_liveness.py](dbg_fcbnd_liveness.py)）→ **非 M32 純死**：layout-key（含 `boundary_bad` 項）確實挑到更好的 boundary aspect（異於 M32 per-block greedy-area 反向的 0.000%），M36「更好 arbiter 翻盤」方向**確認正確**。**但加權增益 below bar**：贏案全是低權重小/中案（63/49/11/71/36/23/16/6…），**高權重硬案 89/97/85/82/88 在任何 ratio（0.333..4.0）零移動**——boundary-capacity 主導的硬案已被 LR/TB default + anchored/MIB 機制最優處理，build-time layout-key 的 boundary 估計不足以贏實際 vBd。三 stack：fc_gm+BND **+0.034%**（best）、fc_pin+BND +0.023%、anc_mib+BND +0.017%，全 <0.05% bar → **code 已 revert**（live-but-below-bar；要重試 = 3 行：static 宣告 + gate `(boundary==0||FREE_CLUSTER_BND>0)` + env parse，鏡像 FREE_ANCHORED_BND）。**連續第二個 genuine below-bar（接 M38）→「預判死翻盤」三連（M35-37）結束、free-aspect 六子軸確定收束**
...
89 **~1.523**（M37 MIB wide-reshape 從 M35 的 1.5538 再降，仍最高；preplaced boundary 撐壞 outline）、85 ~1.536（M32 LR4.5）、65 **~1.690**（M35 從 1.7325）、62 1.5227、88 **~1.385**（M36 從 1.412）、97 **~1.199**（M36 ungate-boundary 從 1.228）、82 ~1.363（M34）、52 **~1.361**（M35 從 1.4071）、61 **~1.303**（M37 從 1.3134）、79 ~1.247（M34）、66 ~1.270（M34）、91 1.3481。硬 case（89/85/62）= preplaced boundary 幾何強迫；**M33→M37 證實成員形狀（cluster-member + anchored-member + MIB-member aspect，含寬 ratio + ungate boundary）+ tight frame 能顯著鬆動 89/82/88/97/79/66/65/61/52**（非 packer 架構問題，是成員形狀；連 MIB 共享形狀都能優化，再推翻「自由度低」預判）。
...
- **aspect 軸 MIB-member（M37）**：`ICCAD_MIB_ASPECT=r`（`apply_safe_mib_dims` 的無-master square-fallback 群——共享 `sqrt(avg)` 方形——reshape 成同面積 `avg` 的共享矩形 `w=sqrt(avg·r),h=avg/w`；保共享形狀 → MIB violation 維持 0、all-interior gated（boundary 成員不可拆共享形狀 + 傷 edge capacity）；預設 1.0=方形=bit-identical；套在 `solve()` `apply_safe_mib_dims()` 內）。**M37 ship 1.3269**，1 profile `fc_anchored_mib_pin_tight`（MIB=5.0 疊 M36 anchored recipe，+0.110% oracle-min，破最難案 89→**1.5232** + 61）。⚠️ uniform、小群（14/100 案可 reshape、各 1 群 size 3-6）；**standalone/pin_tight/gm stack 全死，win 須疊 anchored 旋鈕放大**（89 在 pin_tight=0）；**共振峰 wide 5.0**（4.0 +0.098%、6.0 退步、3.0/3.5 弱）；tall 0.25→79 +0.027% below bar；build-time reshape → 加寬 wall-free。第三度推翻「uniform 小群 = 死」預判（同 M35/M36 模式）。**殘渣**：per-group MIB 搜尋（M34-style，可同 profile 收 89-wide+79-tall）未做、ROI 低
...
- `optimizer_constructive.py` 🏆 — 40-prof portfolio + shapely proxy(_RH=1.4)；M25 剪 18 條（`[M25-pruned]` 註解可復原）
