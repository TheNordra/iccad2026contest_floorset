...
- **現況：CONVERGED @ 1.3269**（領先組員所有 legit ~18.1%、avg 9.89s、100/100 feasible）。**所有 in-env lever 探畢**——packer/order（M26-27 天花板）+ free-aspect 六子軸（M29-M37 全 ship/死）+ reconstruction（M40 RED-confirmed）。要再追分須**全新 insight**（見「未來發展方向」）。逐版細節在 git log（commit M29-M40）+ memory（`[[m29-tree-decoder]]`…`[[m40-reconstruction-red]]`）。
...
### 🏆 最佳：Total Score = 1.3269（M37 MIB-aspect wide-reshape, 40-prof, 2026-06-22；avg 9.89s, est_wall OS16-bound, 100/100 feasible）
...
## 未來發展方向（post-convergence）

> 六子軸 family CLOSED + reconstruction RED → **無 incremental lever**。1.3269 是合理停點。依 ROI：

1. **收束本題（現行結論）**：1.3269 領先 legit ~18.1%、runtime 安全。要再追分須**全新 insight**（非加 lever、非重寫 slicing placer）——目前沒有。
2. **收斂硬化提交（submission hardening，唯一仍有用的工程；對「有後續輪次」實用、不追分）**：
   - **proxy generalization**：`_RH=1.4` 由本地 validation sweep 來——hidden test 分佈不同時是否仍 oracle-min？離線可驗（`rh_sweep.py` 看 1.3-1.6 平台寬度；窄=有 overfit 風險）。
   - **runtime/timeout 安全邊際**：大案 n>110 ~20-26s（OS16 牆是 est_wall 主項），`_run_profile` timeout=120s；hidden test 若有更大 n 需查餘裕，必要時對超大案降 OS16 K。
   - **feasibility 保證**：目前 100/100；portfolio 全 fail 時退 `python_sa_solve` fallback——確認 fallback 仍 feasible（少跑）。
3. **等全新 insight**：新增分數須非 lever 的新角度（新 constraint 結構洞察 / 新表徵），非既有軸的延伸。
...
- `rh_sweep.py`（真值快取 + 掃 _RH/proxy）、`portfolio_ceiling.py`（oracle 天花板）、`profile_audit.py`（M25 池審計：win tally/LOO/cpu）
...
- `optimizer_constructive.py` 🏆 — 40-prof portfolio + shapely proxy(_RH=1.4)；pruned profile 標 `[Mxx-pruned]` 註解可復原
