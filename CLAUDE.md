...
- **現況：M41（2026-06-25）開全新 RuntimeFactor 軸（與 quality 正交，被本地 eval 強制 RF=1.0 藏住）；M42（2026-06-26）第二槍；M45（2026-07-02）第三槍 per-band tiers + cores-adaptive；M46（2026-07-03）第四槍 C++ hot-path exact 加速；M47（2026-07-04）第五槍 wrapper overhead（proxy overlap + tolist internals）；M48（2026-07-04）送件 hardening（編譯鏈+binary smoke、solve() 三層安全網、冷啟動 dry-run；零分數變更）**。本地分 1.3269→**1.3277**（RF=1.0 fiction，+0.06%、之後恆定）：M41 砍 6 隻 swap profiles（OS16×3/OS8×2/OM8，大案 18-20s 純 runtime 死重）→ avg 9.89→5.90s；**M42 再砍 21 隻「不贏任何 n>100 案」的 build profiles（per-big-n 冗餘，非全域 LOO）→ avg 5.90→4.73s（−20%）、big-case wall n=120 砍半 15.6→8.0s、quality BIT-IDENTICAL（local 仍 1.3277、20 案全 median-independent WIN）**、投影 real 疊加 M41 共 **~−22%**（@ M=11、robust 跨 M∈[6,20]）、100/100 feasible；**M46 再 exact 加速 pack 熱路徑（opt-A/B/C，全池 4100/4100 positions bit-identical）→ avg 4.73→2.70s、@12c M=11 投影 1.1473→0.9757（再 −15%）、零下行（純加速無 +0.06% local 代價）**；**M47 堵 wrapper overhead（計時窗內串行 shapely proxy = n>100 wall 的 29.2%、rf_score_model 從未建模）：proxy 改 as_completed 與 pool 重疊 + proxy 內部 tolist 化（300/300 逐位 gate）→ avg 2.70→1.85s、n>100 total −34.8%、tail 29.2%→0.4%、eval 1.3277 逐位——把投影 0.9757 的兌現誤差從 +8~11% 堵到 ~2%**。**quality 軸仍 converged**（packer/order M26-27 + free-aspect 六子軸 M29-M37 + reconstruction M40-RED 全探畢）；RF 軸唯一前提是「官方真套 RF」（否則只損 +0.06%，極不對稱賭注）。逐版細節 git log + memory（`[[m46-wall-setter-speedup]]`、`[[m45-band-tiers]]`、`[[m42-runtime-2nd-order]]`…`[[m29-tree-decoder]]`）。
...
## 🔑 戰略結論：quality 三面天花板皆探畢；**RF 軸（M41+M42+M45+M46）四槍已 ship**
...
- ⚠️ **eval 實際 ~4 分鐘（M47 後）**（100 案 serial、avg 1.85s，大案 n>110 ~2-4.5s；M46 時代 ~6 分、M42 ~8 分、M41 前 ~15-17 分）；background 用 harness `run_in_background`（完成通知）；`> file 2>&1` 在 PowerShell 對 native exe 印 cosmetic `NativeCommandError`（無害）

```powershell
# 編譯（Bash 工具寫 .exe 會失敗，務必用 PowerShell）
& "C:\msys64\ucrt64\bin\g++.exe" -O3 -std=c++17 -o constructive.exe constructive.cpp

# 官方 portfolio eval（~6 分鐘，確定性）
...
- **`m46_stage_probe.py` + `constructive_m46.cpp/.exe`（M46 probe，永不 ship）**：計時儀裝副本（`ICCAD_STAGE_TIMING=1` → stderr TIMING 行）+ 四模式 driver（`verify` 12 組 byte-compare / `verify100` 全 100 案×6 profile + per-band α / `diff` 差分旋鈕 / `timing` stage 預算）。opt-A/B/C 已於 M46（2026-07-03）ship 併入 `constructive.cpp`；副本保留儀裝供 stage 分析，verify/verify100 現在=比對 merged vs probe（ship 時 600/600 PASS）
