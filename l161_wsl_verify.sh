...
# Run INSIDE WSL:  wsl -d Ubuntu -- bash /mnt/c/ICCAD_ml/ship_final/l161_wsl_verify.sh
set -u
V=$HOME/iccadvenv/bin/python
R=/mnt/c/ICCAD_ml/ship_final
export L117_WORK=$HOME/l161
TAR=$R/build_submission/cadc1075.tar.gz
cd "$R" || exit 1
mkdir -p "$L117_WORK"

echo "== L161 Linux verify -- n-set 89  $(date -u +%FT%TZ)  nproc=$(nproc)  $(uname -r)"
...
echo; echo "########## BUDGET ##########"
"$V" l153_budget.py "$BASE" "$CTRL" | tee "$R/l161_budget_linux.txt"
B=$(grep '^BUDGET' "$R/l161_budget_linux.txt" | awk '{print $2}')
echo "   budget = $B"
...
echo; echo "L161_WSL_DONE $(date -u +%FT%TZ)"
