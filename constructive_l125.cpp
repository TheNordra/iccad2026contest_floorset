...
// L125 bounded beam (PROBE ONLY, never in the shipping source).  The item loop in
// pack_in_frame commits the single best candidate per item; the beam keeps W
// partial layouts across the first K committing items, then lets each complete
// greedily and picks the completion the frame loop itself would prefer.  Cost is
// therefore ~W x a normal pack (completion dominates), which is the shape L125-B0
// priced: W=2 is +0.0002% weighted dRF if the beamed profile is chosen, W=3 eats
// two thirds of the 0.30% bar, W=4 is dead.
// ICCAD_BEAM=0 (default) leaves pack_in_frame untouched -> bit-identical.
// ICCAD_BEAM=1 with ICCAD_BEAM_W=1 runs the beam code as plain greedy and must
// ALSO be bit-identical -- that is the gate on the duplicated placement logic.
static int BEAM_ON = 0;    // ICCAD_BEAM
static int BEAM_W  = 2;    // ICCAD_BEAM_W  beam width
static int BEAM_K  = 8;    // ICCAD_BEAM_K  number of branching item levels
static int BEAM_SEL = 0;   // ICCAD_BEAM_SEL 0=layout_score after the 3 nudges, 1=raw
...
// L125: declared here, defined after pack_in_frame; needs the scoring helpers
// that live further down the file.
static bool pack_in_frame_beam(double fw,double fh,const vector<Item>& items,vector<XYWH>& out);
...
static bool pack_in_frame(double fw,double fh,const vector<Item>& items,vector<XYWH>& out){
    if (BEAM_ON) return pack_in_frame_beam(fw,fh,items,out);   // L125 (default off)
...
// ─── L125 bounded beam ────────────────────────────────────────────────────────
// The same placement arithmetic as pack_in_frame above, restructured so W partial
// layouts can be carried instead of one.  Deliberately a separate function: the
// shipped path stays byte-for-byte untouched, and the duplicated logic is gated by
// requiring ICCAD_BEAM=1 ICCAD_BEAM_W=1 to be bit-identical to greedy.
//
// Prefix cost of a partial layout is  bbox_area + the non-area terms the greedy
// step score already carries (anchor / wire / boundary / tie-break).  Within one
// parent this ranks exactly as the greedy step score does -- greedy's `area` term
// IS the child's bbox area -- so W=1 reproduces the greedy pick including its
// first-minimum tie-break.  Across parents it is the only comparable prefix
// quantity there is.
//
// Risk recorded BEFORE the code was written: layout_score has no monotone
// relationship to final cost on partial layouts (that is what killed the
// frame-level B&B -- guide.swap runs unconditionally and three nudges break bbox
// monotonicity), so this prefix cost is a heuristic and top-W need not retain the
// eventual winner.  A beam tolerates that better than B&B because it keeps W, not
// 1, but this is where it fails if it fails.
struct BeamNode {
    vector<XYWH> out;
    vector<XYWH> rects;
    vector<char> done;
    double bx0,by0,bx1,by1; int bc;
    M46Grid g46;
    double acc;   // accumulated non-area step terms
    double g;     // prefix cost = bbox area + acc
};
// The bbox is file-scope state (bbox_area_with reads it) and M46Grid holds a
// pointer to its node's own rects, which moves when `live` reallocates -> both are
// re-bound on every use rather than trusted across a copy.
static inline void beam_use(BeamNode& s){
    bminx=s.bx0; bminy=s.by0; bmaxx=s.bx1; bmaxy=s.by1; bcount=s.bc;
    s.g46.rp=&s.rects;
}
static inline void beam_save(BeamNode& s){
    s.bx0=bminx; s.by0=bminy; s.bx1=bmaxx; s.by1=bmaxy; s.bc=bcount;
}
struct BeamCand {
    int node;          // parent index in `live`
    double x,y,w,h;    // w,h only used by the FREE_ASPECT single path
    int single;        // block id for that path, else -1
    double area;       // the bbox-area part of the step score
    double g;          // parent.acc + step score
};
static unsigned long long beam_key(const BeamNode& s){
    unsigned long long h=1469598103934665603ULL;
    for (int i=0;i<N;i++){
        if (!s.done[i]) continue;
        long long q[4]={(long long)llround(s.out[i].x*1e6),(long long)llround(s.out[i].y*1e6),
                        (long long)llround(s.out[i].w*1e6),(long long)llround(s.out[i].h*1e6)};
        for (int k=0;k<4;k++){ h^=(unsigned long long)q[k]; h*=1099511628211ULL; }
        h^=(unsigned long long)i; h*=1099511628211ULL;
    }
    return h;
}
// every legal placement of `it` in state s, in the greedy's own enumeration order
static void beam_expand(BeamNode& s,int ni,const Item& it,double fw,double fh,
                        double ww,vector<BeamCand>& outc){
    beam_use(s);
    const vector<XYWH>& out=s.out; const vector<char>& done=s.done;
    if (FREE_ASPECT>0 && it.blocks.size()==1){
        int sb=it.blocks[0];
        if (!blocks[sb].is_fixed && !blocks[sb].is_preplaced && blocks[sb].mib==0
            && blocks[sb].boundary==0 && blocks[sb].area>0){
            ...
            for (double r:FREE_RATIOS){
                double IW=sqrt(A*r), IH=A/IW;
                Item t=it; t.w=IW; t.h=IH;        // single: offs[0]=(0,0)
                auto cands=item_candidates(t,fw,fh,s.rects);
                if (GUIDE_MED){
                    vector<pair<double,double>> xs, ys;
                    for (auto& nb:b2b_adj[sb]){
                        double ncx,ncy;
                ...
                for (auto&c:cands){
                    double x=c.first,y=c.second;
                    if (x<-TOL||y<-TOL||x+IW>fw+TOL||y+IH>fh+TOL) continue;
                    if (s.g46.overlaps(x,y,IW,IH)) continue;
                    double cx=x+IW/2, cy=y+IH/2;
                    double ad=it.aw>0?fabs(cx-it.ax)+fabs(cy-it.ay):0.0;
                    double area=bbox_area_with(x,y,IW,IH), wire=0.0;
                    for (auto& nb:b2b_adj[sb]){
                    ...
                    for (auto& pn:p2b_adj[sb])
                        wire+=pn.second*(fabs(cx-pins[pn.first].first)+fabs(cy-pins[pn.first].second));
                    double score=area+ANCHOR_W*ad+ww*WIRE_MULT*wire+1e-3*y+1e-4*x;
                    outc.push_back({ni,x,y,IW,IH,sb,area,s.acc+score});
                }
            }
            return;
        }
    }
    auto cands=item_candidates(it,fw,fh,s.rects);
    if (GUIDE_MED){
        vector<pair<double,double>> xs, ys;
        for (size_t k=0;k<it.blocks.size();k++){
            int b=it.blocks[k];
    ...
    for (auto&c:cands){
        ...
        for (size_t k=0;k<it.blocks.size()&&!ov;k++){
            int b=it.blocks[k]; double rx=x+it.offs[k].first, ry=y+it.offs[k].second;
            double bw=dims[b].first, bh=dims[b].second;
            if (s.g46.overlaps(rx,ry,bw,bh)){ ov=true; break; }
        }
        if (ov) continue;
        double cx=x+it.w/2, cy=y+it.h/2;
        double ad=it.aw>0?fabs(cx-it.ax)+fabs(cy-it.ay):0.0;
        ...
        double score=area+ANCHOR_W*ad+ww*WIRE_MULT*wire+BP_W*bp+1e-3*y+1e-4*x;
        outc.push_back({ni,x,y,0.0,0.0,-1,area,s.acc+score});
    }
}
static void beam_apply(BeamNode& s,const Item& it,const BeamCand& c){
    beam_use(s);
    if (c.single>=0){
        int b=c.single;
        s.out[b]={c.x,c.y,c.w,c.h}; s.rects.push_back({c.x,c.y,c.w,c.h});
        s.g46.add((int)s.rects.size()-1); bbox_add(c.x,c.y,c.w,c.h); s.done[b]=1;
    } else {
        for (size_t k=0;k<it.blocks.size();k++){
            int b=it.blocks[k]; double rx=c.x+it.offs[k].first, ry=c.y+it.offs[k].second;
            double bw=dims[b].first, bh=dims[b].second;
            s.out[b]={rx,ry,bw,bh}; s.rects.push_back({rx,ry,bw,bh});
            s.g46.add((int)s.rects.size()-1); bbox_add(rx,ry,bw,bh); s.done[b]=1;
        }
    }
    beam_save(s);
    s.acc=c.g-c.area;   // parent.acc + (step score - its bbox-area part)
    s.g=c.g;
}
static bool pack_in_frame_beam(double fw,double fh,const vector<Item>& items,vector<XYWH>& out_final){
    double ww=(N>=116)?150.0:(N>=100?70.0:50.0);
    BeamNode s0;
    s0.out=pos; s0.done.assign(N,0); s0.acc=0.0; s0.g=0.0;
    bbox_reset(); s0.g46.init(fw,fh,&s0.rects);
    for (int i=0;i<N;i++) if (placed[i]){
        if (pos[i].x<-TOL||pos[i].y<-TOL||pos[i].x+pos[i].w>fw+TOL||pos[i].y+pos[i].h>fh+TOL) return false;
        for (const XYWH&r:s0.rects) if (rect_overlap(pos[i].x,pos[i].y,pos[i].w,pos[i].h,r.x,r.y,r.w,r.h)) return false;
        s0.rects.push_back(pos[i]); s0.g46.add((int)s0.rects.size()-1);
        bbox_add(pos[i].x,pos[i].y,pos[i].w,pos[i].h); s0.done[i]=1;
    }
    // Anchored first-pass: deterministic and identical for every beam node, so it
    // runs once, before the beam exists.
    for (const AnchoredCluster& ac:anchored_clusters){
        vector<XYWH> cluster_rects;
        for (int b:ac.preplaced) if (s0.done[b]) cluster_rects.push_back(s0.out[b]);
        vector<int> mov=ac.movable;
        sort(mov.begin(),mov.end(),[](int a,int b){
            int ba=block_boundary_score(a), bb=block_boundary_score(b);
            if (ba!=bb) return ba>bb;
        ...
        for (int b:mov){
            ...
            for (double r:ratios){
                ...
                for (auto&c:cands){
                    double x=c.first, y=c.second;
                    if (x<-TOL||y<-TOL||x+cw>fw+TOL||y+ch>fh+TOL) continue;
                    if (s0.g46.overlaps(x,y,cw,ch)) continue;
                    double cx=x+cw/2, cy=y+ch/2;
                    double ad=anchors[b].w>0?fabs(cx-anchors[b].x)+fabs(cy-anchors[b].y):0.0;
                    int bp=boundary_penalty_est(b,x,y,cw,ch,fw,fh);
                    double area=bbox_area_with(x,y,cw,ch), wire=0.0;
                    if (bp==0){
                        for (auto& nb:b2b_adj[b]){
                            double ncx,ncy;
                            if (s0.done[nb.first]){ ncx=s0.out[nb.first].x+s0.out[nb.first].w/2; ncy=s0.out[nb.first].y+s0.out[nb.first].h/2; }
                            else if (use_prev){ ncx=prev_pos[nb.first].x+prev_pos[nb.first].w/2; ncy=prev_pos[nb.first].y+prev_pos[nb.first].h/2; }
                            else continue;
                            wire+=nb.second*(fabs(cx-ncx)+fabs(cy-ncy));
                        }
            ...
            if (found){
                s0.out[b]={bx,by,bw,bh}; s0.rects.push_back({bx,by,bw,bh}); s0.g46.add((int)s0.rects.size()-1);
                bbox_add(bx,by,bw,bh); s0.done[b]=1; cluster_rects.push_back({bx,by,bw,bh});
            }
        }
    }
    beam_save(s0);

    vector<BeamNode> live; live.push_back(move(s0));
    int branch_left=BEAM_K;
    vector<BeamCand> pool, cs;
    for (const Item& it:items){
        // Every live node has placed exactly the same items, so the skip tests
        // read the same on all of them.
        bool all_done=true; for (int b:it.blocks) if(!live[0].done[b]){ all_done=false; break; }
        if (all_done) continue;
        bool any_done=false; for (int b:it.blocks) if(live[0].done[b]){ any_done=true; break; }
        if (any_done) continue;
        bool branch = (branch_left>0 && BEAM_W>1);
        pool.clear();
        for (int ni=0;ni<(int)live.size();ni++){
            cs.clear();
            beam_expand(live[ni],ni,it,fw,fh,ww,cs);
            if (cs.empty()) continue;                       // this node dies here
            int keep = branch ? BEAM_W : 1;
            stable_sort(cs.begin(),cs.end(),
                        [](const BeamCand&a,const BeamCand&b){ return a.g<b.g; });
            if ((int)cs.size()>keep) cs.resize(keep);
            for (auto&c:cs) pool.push_back(c);
        }
        if (pool.empty()) return false;                     // every node died => greedy would too
        vector<BeamNode> next;
        if (branch){
            stable_sort(pool.begin(),pool.end(),
                        [](const BeamCand&a,const BeamCand&b){ return a.g<b.g; });
            set<unsigned long long> seen;
            for (auto&c:pool){
                if ((int)next.size()>=BEAM_W) break;
                BeamNode s=live[c.node];
                beam_apply(s,it,c);
                if (!seen.insert(beam_key(s)).second) continue;   // duplicate layout
                next.push_back(move(s));
            }
            branch_left--;
        } else {
            for (auto&c:pool){ BeamNode s=live[c.node]; beam_apply(s,it,c); next.push_back(move(s)); }
        }
        live.swap(next);
    }
    if (live.empty()) return false;
    // Select the completion the frame loop itself would prefer: run_frame applies
    // the three nudges and then compares layout_score, so score the nudged copy.
    int bi=0;
    if ((int)live.size()>1){
        double best=1e300;
        for (int i=0;i<(int)live.size();i++){
            vector<XYWH> q=live[i].out;
            if (BEAM_SEL==0){
                final_boundary_nudge(q); final_group_boundary_nudge(q); final_single_edge_escape(q);
            }
            double sc=layout_score(q);
            if (sc<best){ best=sc; bi=i; }
        }
    }
    out_final=live[bi].out;
    return true;
}
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_BEAM")){ int v=atoi(e); if (v>0) BEAM_ON=v; }                       // L125
    if (const char* e=getenv("ICCAD_BEAM_W")){ int v=atoi(e); if (v>0) BEAM_W=v; }
    if (const char* e=getenv("ICCAD_BEAM_K")){ int v=atoi(e); if (v>=0) BEAM_K=v; }
    if (const char* e=getenv("ICCAD_BEAM_SEL")){ int v=atoi(e); if (v>=0) BEAM_SEL=v; }
