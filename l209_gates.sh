#!/bin/sh
# L209 -- in-set gates for the L196 tree WITH ROUTE A OFF (L205 decision)
#
# Route A is verified result-neutral, so every arm here must reproduce its
# L199 counterpart BIT-FOR-BIT. That is the point: the change is supposed to
...
set -u
LOCK=/c/ICCAD_ml/ship_final/.l209.lock
...
run () {
  tag="$1"; shift
  rm -f "../l209_${tag}_stats.txt"
  env ICCAD_ADAPTIVE_CORES=48 ICCAD_SHAPE_LP_STATS="../l209_${tag}_stats.txt" "$@" \
    "$PY" -u iccad2026_evaluate.py --evaluate ../optimizer_constructive.py \
    -o "../results_L209_${tag}.json" > "../l209_${tag}.log" 2>&1
  rc=$?
  n=$(wc -l < "../l209_${tag}_stats.txt" 2>/dev/null || echo 0)
  echo "$tag exit=$rc  LPran=${n}  SAfallback=$(grep -c 'SA fallback' "../l209_${tag}.log")  scipy=$(grep -c 'scipy] source=' "../l209_${tag}.log")"
}
run det1
run det2
