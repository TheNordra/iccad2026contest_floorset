...
// ─── L144-V1 twin candidate: ICCAD_BND_ABUT ("abut or corner") ───────────────
// EDGE_RUN above is RED: a *gradient* along the edge pins every boundary item to
// the low corner, which is a wire-term tax on the FIRST item of each edge and
// buys nothing (count 26 -> 26/27/28/30 at every weight 30..30000).
//
// V1 replaces the gradient with a DISCRETE gap penalty. A compliant (bp==0)
// candidate pays w only if, on each side it is constrained to, its along-edge
// interval touches neither
//   (a) a frame corner of that edge (interval starts at 0 or ends at the frame
//       extent), nor
//   (b) the along-edge interval of a block already placed AND actually sitting
//       on that same side.
// So the first item on an edge is free to follow the wire term anywhere that
// reaches a corner, and later items are only stopped from leaving unusable holes
// between themselves and what is already there.
//
// Same lexicographic guard as EDGE_RUN: selection is on (bp>0, score + penalty),
// so a compliant candidate can never lose to a non-compliant one at any weight.
// w=0 disables the whole path => bit-identical to constructive.cpp.
static double BND_ABUT_W = 0.0;
// V1 liveness counters. Written only when BND_ABUT_W>0, printed to stderr only,
// read back by nothing => the OFF path is untouched.
static long TRV_place = 0;      // boundary-item placements with a compliant cand.
static long TRV_pen   = 0;      //   ... where the WINNER paid the abut penalty
static long TRV_flip  = 0;      //   ... where the penalty CHANGED the winner
...
static bool pack_in_frame(double fw,double fh,const vector<Item>& items,vector<XYWH>& out){
    ...
    for (const Item& it:items){
        ...
        // ── L144-V1 "abut or corner": along-edge intervals of the blocks that are
        // already placed AND actually sitting on each side, built once per item.
        bool abut_on=false;
        static const int ABUT_BIT[4]={B_LEFT,B_RIGHT,B_TOP,B_BOTTOM};
        vector<pair<double,double>> occ[4];
        if (BND_ABUT_W>0){
            for (int b : it.blocks) if (blocks[b].boundary) { abut_on=true; break; }
            if (abut_on) for (int j=0;j<N;j++){
                if (!done[j]) continue;
                int cj=blocks[j].boundary; if (!cj) continue;
                const XYWH& q=out[j];
                if ((cj&B_LEFT)   && fabs(q.x-0.0)<=TOL)    occ[0].push_back({q.y,q.y+q.h});
                if ((cj&B_RIGHT)  && fabs(q.x+q.w-fw)<=TOL) occ[1].push_back({q.y,q.y+q.h});
                if ((cj&B_TOP)    && fabs(q.y+q.h-fh)<=TOL) occ[2].push_back({q.x,q.x+q.w});
                if ((cj&B_BOTTOM) && fabs(q.y-0.0)<=TOL)    occ[3].push_back({q.x,q.x+q.w});
            }
        }
        auto abut_ok=[&](double ox,double oy)->bool{
            for (size_t k=0;k<it.blocks.size();k++){
                int b=it.blocks[k]; int code=blocks[b].boundary; if (!code) continue;
                double rx=ox+it.offs[k].first, ry=oy+it.offs[k].second;
                double bw2=dims[b].first, bh2=dims[b].second;
                for (int s=0;s<4;s++){
                    if (!(code&ABUT_BIT[s])) continue;
                    bool lr=(s<2);
                    double lo = lr? ry : rx;
                    double hi = lr? ry+bh2 : rx+bw2;
                    double ext= lr? fh : fw;
                    if (lo<=TOL || hi>=ext-TOL) continue;        // (a) frame corner
                    bool touch=false;
                    for (auto&iv:occ[s])
                        if (hi>=iv.first-TOL && lo<=iv.second+TOL){ touch=true; break; }
                    if (!touch) return false;                    // (b) no neighbour
                }
            }
            return true;
        };
        double alt_best=1e300, alt_x=0, alt_y=0; bool alt_found=false, chosen_pen=false;
        for (auto&c:cands){
            double x=c.first,y=c.second;
            ...
            if (run_side || abut_on){
                ...
                int cls = (bp>0) ? 1 : 0;
                double s2 = score; bool pen=false;
                if (cls==0){
                    if (run_side){
                    ...
                    if (abut_on && !abut_ok(x,y)){ s2 += BND_ABUT_W; pen=true; }  // L144-V1
                    if (abut_on && score<alt_best){ alt_best=score; alt_x=x; alt_y=y; alt_found=true; }
                }
                if (cls<best_cls || (cls==best_cls && s2<best)){
                    best_cls=cls; best=s2; bx=x; by=y; found=true; chosen_pen=pen;
        ...
        if (abut_on && alt_found){                                    // L144-V1 liveness
            TRV_place++; if (chosen_pen) TRV_pen++;
            if (fabs(bx-alt_x)>TOL || fabs(by-alt_y)>TOL) TRV_flip++;
        }
        if (!found) return false;
        for (size_t k=0;k<it.blocks.size();k++){
...
static void solve() {
    ...
    if (BND_ABUT_W>0)
        fprintf(stderr,"ABUTLIVE w=%g place=%ld pen=%ld flip=%ld\n",
                BND_ABUT_W,TRV_place,TRV_pen,TRV_flip);
    if (BND_TRACE)
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_BND_ABUT"))     { double v=atof(e); if (v>0) BND_ABUT_W=v; }  // L144-V1
