...
def score_sel(selector):
    return compute_total_score([recs[i][0][selector(i)]["cost"] for i in range(len(ns))], ns)

print("\n=== per-profile Total Score ===")
for p in PNAMES:
    print(f"  {p:11s} {compute_total_score([recs[i][0][p]['cost'] for i in range(len(ns))], ns):.4f}")

orc = score_sel(lambda i: min(PNAMES, key=lambda p: recs[i][0][p]["cost"]))
print(f"\n  {'ORACLE':11s} {orc:.4f}   <- ceiling")

# ---- baseline-free proxy search ----
def proxy_pick(i, RH, vkey="vrel"):
    rec, sumA = recs[i]
    A = 1.035 * max(sumA, 1e-9)
    hmin = min(rec[p]["hpwl"] for p in PNAMES) or 1.0
    vals = {p: (rec[p]["area"]/A + RH*rec[p]["hpwl"]/hmin) * math.exp(2.0*rec[p][vkey])
            for p in PNAMES}
    return min(PNAMES, key=lambda p: vals[p])

print("\n=== proxy with HARNESS vrel (offline ideal) ===")
for RH in (0.5, 1.0, 2.0):
    print(f"  RH={RH:<4} {score_sel(lambda i, RH=RH: proxy_pick(i, RH, 'vrel')):.4f}")
print("=== proxy with C++ vrel (what the live wrapper does) ===")
for RH in (0.5, 1.0, 2.0):
    print(f"  RH={RH:<4} {score_sel(lambda i, RH=RH: proxy_pick(i, RH, 'vrel_c')):.4f}")

# how often do harness vrel and C++ vrel disagree enough to flip the pick?
flip = sum(1 for i in range(len(ns))
           if proxy_pick(i, 1.0, "vrel") != proxy_pick(i, 1.0, "vrel_c"))
print(f"\nC++ vs harness vrel flips pick on {flip}/{len(ns)} cases")
