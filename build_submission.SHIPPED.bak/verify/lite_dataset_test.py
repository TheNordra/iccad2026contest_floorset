...
def download_dataset(root):
    ...
    f_name = os.path.join(root, 'LiteTensorDataTest.tgz')
...
def is_dataset_downloaded(root):
    return len(glob.glob(os.path.join(root, 'LiteTensorDataTest', 'config*'))) >= 100
...
class FloorplanDatasetLiteTest(Dataset):
    def __init__(self, root):
        ...
        for worker_idx in partition_range:
            config_dir = os.path.join(root, f'LiteTensorDataTest/config_{worker_idx}')
