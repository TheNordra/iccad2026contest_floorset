...
Run (PowerShell):
  ...
  <python> -u m79_knob_cloud_probe.py greedy [R] [KMAX]   # writes m81_vectors_recentre.json
...
def build_cloud(R):
    ...
    # M81 arm (1): re-centre. The perturbation half has always mutated the
    # SHIPPED 41, but M80 already proved its own 8 are the best region known,
    # so this makes them the centres and does a second round of local search
    # there. Overfitting risk is real (doubling down on an in-sample winner),
    # which is exactly what the 5-fold held-out gate is for.
    src = [p for p in oc._M80_EXTRA
...
def _write_vectors(order, R, kind):
    ...
    out = _DIR / "m81_vectors_recentre.json"
...
def mode_dump(R, order):
    """Regenerate a named pick order into m81_vectors_recentre.json without re-running the
