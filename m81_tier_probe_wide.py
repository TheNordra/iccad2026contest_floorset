...
# M81 arm (2): the widened-clamp vector set. Same machinery, same
# caches (keyed by (sample, case, profile-hash) so the two arms
# cannot collide), only the 12 candidate vectors differ.
VECFILE = _DIR / "m81_vectors_wide.json"
...
def mode_score(args):
    ...
    p = _DIR / f"results_M81wide_oos_{sample}_c{cores}.json"
