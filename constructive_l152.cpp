...
// ─── L152 probe flags (default 0 = bit-identical) ────────────────────────────
// R2: a compound item holding BOTH an L member (flush at ox=0) and an R member
// (flush at ox+bw=it.w) needs x=0 and x+it.w=bbox_max at once, i.e. nothing else
// may extend past its right edge. frame_candidates() never sizes a frame to an
// item's width, so today that is unsatisfiable in every frame the placer builds
// (measured: 19 clusters, 24 violations, 14.7x the rate of other clusters).
static int BND_FRAME_ITEM = 0;   // ICCAD_BND_FRAME_ITEM
static vector<double> L152_REQ_W, L152_REQ_H;   // filled per solve, read by frame_candidates
// R3: the EXPOSE snap variant is all-or-nothing -- one overlap discards the whole
// candidate, so a member whose offset leaves it inside the item's own bbox never
// reaches the frame edge (measured: every non-flush member violates, 28 of 28).
static int BND_SNAP_BEST = 0;    // ICCAD_BND_SNAP_BEST
    ...
    auto build_best = [&]()->Item{
        ...
        if (CLUSTER_BND_EXPOSE>0){
            ...
            for (size_t ci=0;ci<nc;ci++){
                ...
                else if (BND_SNAP_BEST>0){
                    // L152 R3: best-effort instead of all-or-nothing. Snap one
                    // member at a time and keep the snap only if it stays
                    // overlap-free; a member that cannot move leaves the others
                    // still snapped rather than reverting the whole candidate.
                    Item p=cands[ci];
                    bool any=false;
                    for (size_t k=0;k<p.blocks.size();k++){
                        int b=p.blocks[k], code=blocks[b].boundary; if (!code) continue;
                        double bw=dims[b].first, bh=dims[b].second;
                        pair<double,double> keep=p.offs[k];
                        if (code&B_LEFT)   p.offs[k].first=0.0;
                        if (code&B_RIGHT)  p.offs[k].first=max(0.0,p.w-bw);
                        if (code&B_BOTTOM) p.offs[k].second=0.0;
                        if (code&B_TOP)    p.offs[k].second=max(0.0,p.h-bh);
                        if (item_has_overlap(p)) p.offs[k]=keep; else any=true;
                    }
                    if (any){ finalize_item(p); cands.push_back(p); }
                }
            }
        }
...
static vector<pair<double,double>> frame_candidates() {
    ...
    // L152 R2: frames sized to an item that must be flush on BOTH opposite
    // edges. The width is forced; the height keeps the same area budget so the
    // extra candidates are not simply bigger outlines.
    if (BND_FRAME_ITEM>0){
        for (double rw : L152_REQ_W){
            double w=max(rw+FRAME_EPS, max(pre_w+FRAME_EPS, max_iw+FRAME_EPS));
            for (double s:scales){
                double h=max(base*base*s*s/w, max(pre_h+FRAME_EPS, max_ih+FRAME_EPS));
                auto key=make_pair((long long)llround(w*1e6),(long long)llround(h*1e6));
                if (seen.insert(key).second) frames.push_back({w,h});
            }
        }
        for (double rh : L152_REQ_H){
            double h=max(rh+FRAME_EPS, max(pre_h+FRAME_EPS, max_ih+FRAME_EPS));
            for (double s:scales){
                double w=max(base*base*s*s/h, max(pre_w+FRAME_EPS, max_iw+FRAME_EPS));
                auto key=make_pair((long long)llround(w*1e6),(long long)llround(h*1e6));
                if (seen.insert(key).second) frames.push_back({w,h});
            }
        }
...
static void solve() {
    ...
    // L152 R2: which item widths/heights must be flush on both opposite edges
    L152_REQ_W.clear(); L152_REQ_H.clear();
    if (BND_FRAME_ITEM>0){
        for (const Item& it : items){
            if (it.blocks.size()<2) continue;
            bool hL=false,hR=false,hB=false,hT=false;
            for (size_t k=0;k<it.blocks.size();k++){
                int code=blocks[it.blocks[k]].boundary; if (!code) continue;
                if (code&B_LEFT) hL=true;   if (code&B_RIGHT) hR=true;
                if (code&B_BOTTOM) hB=true; if (code&B_TOP) hT=true;
            }
            if (hL&&hR) L152_REQ_W.push_back(it.w);
            if (hB&&hT) L152_REQ_H.push_back(it.h);
        }
    }
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_BND_FRAME_ITEM")) BND_FRAME_ITEM=atoi(e);   // L152 R2
    if (const char* e=getenv("ICCAD_BND_SNAP_BEST"))  BND_SNAP_BEST=atoi(e);    // L152 R3
