...
Run (PowerShell):
  ...
  <python> -u m79_knob_cloud_probe.py greedy [R] [KMAX]   # writes m81_vectors_wide.json
...
# M81 arm (2): WIDENED clamps. M80's winning vector #100 sat at
# BP_WEIGHT=274048 against a 300000 ceiling and MIB_ASPECT=0.2338 against a 0.2
# floor -- two of the three knobs that made it win were pinned at the edge of
# the box we were sampling, which is direct evidence the optimum is outside it.
_NUMERIC = {"ICCAD_WIRE_MULT": (0.4, 6.0), "ICCAD_ANCHOR_W": (0.005, 0.50),
            "ICCAD_BP_WEIGHT": (3000, 1000000), "ICCAD_LR_ASPECT": (1.0, 10.0),
            "ICCAD_TB_ASPECT": (0.08, 1.0), "ICCAD_SOFT_ASPECT": (0.35, 3.0),
            "ICCAD_CLUSTER_ASPECT": (0.15, 6.0), "ICCAD_MIB_ASPECT": (0.1, 10.0)}
...
def _write_vectors(order, R, kind):
    ...
    out = _DIR / "m81_vectors_wide.json"
...
def mode_dump(R, order):
    """Regenerate a named pick order into m81_vectors_wide.json without re-running the
