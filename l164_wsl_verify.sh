...
# Run INSIDE WSL:  wsl -d Ubuntu -- bash /mnt/c/ICCAD_ml/ship_final/l164_wsl_verify.sh
set -u
V=$HOME/iccadvenv/bin/python
R=/mnt/c/ICCAD_ml/ship_final
export L117_WORK=$HOME/l164
TAR=$R/build_submission/cadc1075.tar.gz
cd "$R" || exit 1
mkdir -p "$L117_WORK"

echo "== L164 Linux verify -- n-set 89 + vendored scipy  $(date -u +%FT%TZ)  nproc=$(nproc)  $(uname -r)"
...
echo; echo "########## BUDGET ##########"
"$V" l153_budget.py "$BASE" "$CTRL" | tee "$R/l164_budget_linux.txt"
B=$(grep '^BUDGET' "$R/l164_budget_linux.txt" | awk '{print $2}')
echo "   budget = $B"

echo; echo "########## LANE 3 -- 48c, THE SHIPPED DEFAULT, nothing set ##########"
"$V" l117_linux_verify.py final48 "$TAR" --tag t_ship --stats \
     --base "$BASE" --ctrl "$CTRL" --win results_L160_det1.json \
...
echo; echo "L164_WSL_DONE $(date -u +%FT%TZ)"
