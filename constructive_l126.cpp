...
// L126 probe (never shipped as-is): M78's A1 cross-rect candidate, ported onto
// the CURRENT source rather than reused from constructive_m78.cpp -- that
// snapshot predates L108 (no ICCAD_FORCE_FRAME_IDX / FRAME_REPORT, which route A
// needs; a binary that never prints FRAMES makes _room() loop forever) and L124.
// Measuring against a stale baseline would answer a different question.
static int M78_ANCH_CROSS = 0;      // ICCAD_M78_ANCH_CROSS
static int M78_CROSS_K = 6;         // ICCAD_M78_CROSS_K: frontier window
...
static vector<pair<double,double>> adjacent_candidates_for_block(
    ...
    // L126: the shipped set always takes x AND y from the SAME rect, so an origin
    // abutting rect i horizontally while aligning with rect j vertically is
    // unreachable. Bounded to the last K rects (packing order = frontier) so the
    // extra work is O(K^2) per item rather than O(rects^2).
    if (M78_ANCH_CROSS>0){
        size_t m = cluster_rects.size();
        size_t lo = (M78_CROSS_K > 0 && m > (size_t)M78_CROSS_K)
                    ? m - (size_t)M78_CROSS_K : 0;
        for (size_t i=lo;i<m;i++) for (size_t j=lo;j<m;j++){
            if (i==j) continue;
            const XYWH& a=cluster_rects[i]; const XYWH& b=cluster_rects[j];
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_M78_ANCH_CROSS")){ int v=atoi(e); if (v>0) M78_ANCH_CROSS=v; }  // L126
    if (const char* e=getenv("ICCAD_M78_CROSS_K")){ int v=atoi(e); if (v>0) M78_CROSS_K=v; }        // L126
