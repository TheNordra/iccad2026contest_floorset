#!/bin/sh
# L227 -- in-set gates for the L196 tree WITH ROUTE A OFF + the L223 REFINE band (the L211 pool drop is OFF since L224)
#
...
set -u
LOCK=/c/ICCAD_ml/ship_final/.l227.lock
...
run () {
  ...
  rc=$?
  n=$(wc -l < "../l227_${tag}_stats.txt" 2>/dev/null || echo 0)
  echo "$tag exit=$rc  LPran=${n}  SAfallback=$(grep -c 'SA fallback' "../l227_${tag}.log")  scipy=$(grep -c 'scipy] source=' "../l227_${tag}.log")"
}
run det1
run det2
