...
static void solve() {
    ...
    estimate_anchors();
    // M68 (OFFLINE probe, never shipped — mirrors ICCAD_ORDER_FILE at the block below):
    // override per-block MOVABLE anchors with externally supplied (x,y) centers
    // (ML-predicted or fp_sol oracle) to test whether a perfect position SEED lets the
    // greedy+LP chain beat the L3 LP offline anchor 1.2978. Unset -> connectivity anchors
    // unchanged -> byte-identical to shipped. anchors[].w forced >0 so the pull is active
    // even for connectivity-isolated blocks (greedy skips the anchor term when w<=0).
    if (const char* af=getenv("ICCAD_ML_ANCHOR")){
        if (FILE* fp=fopen(af,"r")){
            int id; double ax, ay;
            while (fscanf(fp,"%d %lf %lf",&id,&ax,&ay)==3)
                if (id>=0 && id<N && !blocks[id].is_preplaced)
                    anchors[id]={ax, ay, max(anchors[id].w, 1.0)};
            fclose(fp);
        }
    }
