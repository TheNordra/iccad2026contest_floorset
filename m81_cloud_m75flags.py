...
Run (PowerShell):
  ...
  <python> -u m79_knob_cloud_probe.py greedy [R] [KMAX]   # writes m81_vectors_m75flags.json
...
def _mutate(rng, base):
    ...
    for _ in range(rng.randint(1, 3)):
        ...
        if which < 0.55:
            ...
            # M81 arm (3): the last four are the flags M75 killed. M75 swept
            # them ONE AT A TIME; M80's headline lesson is that dying alone is
            # not dying jointly (its winner #100 combines three separately
            # buried settings). This is the cheapest test of that lesson.
            k = rng.choice(["ICCAD_WIRE_BFS", "ICCAD_BFS_PIN",
                            ...
                            "ICCAD_CLUSTER_BND_CORNER",
                            "ICCAD_ANCHORED_BND_REPACK",
                            "ICCAD_HPWL_SAFE_CLUSTER_SLIDE",
                            "ICCAD_CLUSTER_BND_PERMUTE"])
            if k in p:
                del p[k]
            else:
                p[k] = "1"
...
def _write_vectors(order, R, kind):
    ...
    out = _DIR / "m81_vectors_m75flags.json"
...
def mode_dump(R, order):
    """Regenerate a named pick order into m81_vectors_m75flags.json without re-running the
