...
// ---- L351 Gate 0: NEW FUNCTIONAL FORMS (not new values of existing knobs) ----
// M80 swept the COEFFICIENT space (512 joint vectors, per-case oracle +3.081 %) and its
// held-out figure is saturating. Gate 0 asks the one question that decides Tier-3 item 11:
// does the FORM of the greedy step score reach beyond that coefficient space?
// All four default to 0.0 and L351_ON stays false, so the score expression is LITERALLY
// untouched when off => off-path bit-identical to constructive.exe.
static double L351_WIREUNPL = 0.0;  // wire estimate to UNPLACED neighbours (frame centre)
static double L351_WIRENORM = 0.0;  // normalise wire by the item's own incident weight
static double L351_FILLNORM = 0.0;  // area weight grows as the frame fills
static double L351_ASPECT   = 0.0;  // penalise bbox aspect drift from the frame aspect
static bool   L351_ON       = false;
...
static bool pack_in_frame(double fw,double fh,const vector<Item>& items,vector<XYWH>& out){
    ...
    for (const Item& it:items){
        ...
        for (auto&c:cands){
            ...
            double l351x=0.0;
            if (L351_ON){
                // (1) FORM: the shipped wire term counts only ALREADY-PLACED neighbours,
                //     which is exactly the short-sightedness L276/M78 diagnose. Estimate
                //     the unplaced ones at the frame centre.
                if (L351_WIREUNPL!=0.0){
                    double fu=0.0;
                    for (size_t k=0;k<it.blocks.size();k++){
                        int b=it.blocks[k];
                        double mcx=x+it.offs[k].first+dims[b].first/2;
                        double mcy=y+it.offs[k].second+dims[b].second/2;
                        for (auto& nb:b2b_adj[b]){
                            if (done[nb.first]) continue;
                            if (use_prev) continue;
                            fu+=nb.second*(fabs(mcx-fw/2)+fabs(mcy-fh/2));
                        }
                    }
                    l351x+=ww*L351_WIREUNPL*fu;
                }
                // (2) FORM: normalise wire by the item's incident weight so heavily
                //     connected items do not dominate the ranking by weight alone.
                if (L351_WIRENORM!=0.0){
                    double tw=0.0;
                    for (size_t k=0;k<it.blocks.size();k++){
                        int b=it.blocks[k];
                        for (auto& nb:b2b_adj[b]) tw+=nb.second;
                        for (auto& pn:p2b_adj[b]) tw+=pn.second;
                    }
                    if (tw>0.0) l351x+=ww*L351_WIRENORM*(wire/tw);
                }
                // (3) FORM: make the area term's importance state-dependent -- it should
                //     matter more once the frame is nearly full.
                if (L351_FILLNORM!=0.0){
                    double f=0.0;
                    if (bcount>0) f=((bmaxx-bminx)*(bmaxy-bminy))/max(fw*fh,1e-9);
                    if (f>1.0) f=1.0;
                    l351x+=L351_FILLNORM*f*area;
                }
                // (4) NEW TERM: penalise the resulting bbox drifting away from the
                //     frame's own aspect (the shipped score sees area only).
                if (L351_ASPECT!=0.0){
                    double nx0=x,ny0=y,nx1=x+it.w,ny1=y+it.h;
                    if (bcount>0){ nx0=min(bminx,x); ny0=min(bminy,y);
                                   nx1=max(bmaxx,x+it.w); ny1=max(bmaxy,y+it.h); }
                    double bw2=max(nx1-nx0,1e-9), bh2=max(ny1-ny0,1e-9);
                    double d=log(bw2/bh2)-log(max(fw,1e-9)/max(fh,1e-9));
                    l351x+=L351_ASPECT*area*fabs(d);
                }
            }
            double score=area+ANCHOR_W*ad+ww*WIRE_MULT*wire+BP_W*bp+1e-3*y+1e-4*x;
            if (L351_ON) score+=l351x;
...
int main() {
    ...
    if (const char* e=getenv("ICCAD_L351_WIREUNPL")){ L351_WIREUNPL=atof(e); L351_ON=true; }
    if (const char* e=getenv("ICCAD_L351_WIRENORM")){ L351_WIRENORM=atof(e); L351_ON=true; }
    if (const char* e=getenv("ICCAD_L351_FILLNORM")){ L351_FILLNORM=atof(e); L351_ON=true; }
    if (const char* e=getenv("ICCAD_L351_ASPECT"))  { L351_ASPECT  =atof(e); L351_ON=true; }
