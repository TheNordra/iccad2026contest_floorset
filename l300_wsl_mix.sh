...
export L117_WORK=$HOME/l300
SHIP=$R/build_submission.D/cadc1075.tar.gz
MIX=$R/build_submission.MIX/cadc1075.tar.gz
cd "$R" || exit 1
mkdir -p "$L117_WORK"

echo "== L300 Linux verify (mix)  $(date -u +%FT%TZ)  nproc=$(nproc)  $(uname -r)"
...
"$V" l117_linux_verify.py final48 "$MIX" --tag t_mix --stats \
     --base "$BASE" --ctrl "$CTRL" --win l298_mix.json --budget "${B:-0}" --live-min 1.5
echo "LANE4_RC=$?"
