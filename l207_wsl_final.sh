#!/usr/bin/env bash
# L207 -- Linux verify of the L196 package: _L157_DEPTH flattened to all-1s
...
export L117_WORK=$HOME/l207
TAR=$R/build_submission/cadc1075.tar.gz
cd "$R" || exit 1
mkdir -p "$L117_WORK"

echo "== L207 Linux verify -- L196 package  $(date -u +%FT%TZ)  nproc=$(nproc)"
...
FAIL=0
STG=$(md5sum $R/build_submission/cadc1075/op_wrapper.py | cut -c1-32)
...
[ $? -eq 0 ] || FAIL=1
if [ "$FAIL" -ne 0 ]; then
  echo "L207 ABORT: the tar is not the package we think it is."; exit 1
fi
...
echo; echo "########## BUDGET ##########"
"$V" l153_budget.py "$BASE" "$CTRL" | tee "$R/l207_budget_linux.txt"
B=$(grep '^BUDGET' "$R/l207_budget_linux.txt" | awk '{print $2}')
echo "   budget = $B"
...
echo; echo "L207_DONE $(date -u +%FT%TZ)"
